    ...
    public static Optional<ExecutionHint> fromSqlHint(Optional<SqlNodeList> hints) {
        if (hints.isPresent()) {
            for (SqlHint hint: Iterables.filter(hints.get().getList(),SqlHint.class)) {
                ...
                if (execType.isPresent()) return Optional.of(new ExecutionHint(execType.get()));
            }
        }
        return Optional.empty();
    }

    public SQRLLogicalPlanConverter.Config.ConfigBuilder getConfig(ExecutionPipeline pipeline, SQRLLogicalPlanConverter.Config.ConfigBuilder configBuilder) {
        ...
        switch (execType) {
            ...
            case SERVER:
                ...
                break;
            default: throw new UnsupportedOperationException("Unsupported engine type: " +execType);
        }
        Optional<ExecutionStage> startStage = pipeline.getStage(startType);
        Preconditions.checkArgument(startStage.isPresent(),"Execution type [%s] is not supported by pipeline",execType);
