...
//import org.apache.calcite.jdbc.SchemaWalker;
...
//
//    ContextAwareCalciteSchema nonCachingSchema = new ContextAwareCalciteSchema(null, new CalciteSchema(tableResolver), "");
//
...
//    String query =
////        "select * FROM (SELECT a, ROW_NUMBER() OVER (PARTITION BY id ORDER BY id) as _rownum FROM x) where _rownum = 1";
...
//    hepPlanner.addRule(RuleStub.Config.DEFAULT.toRule());
