...
public class AllOrNoneValueSet
  ...
  public boolean containsValue(Object value) {
    if (!Primitives.wrap(type.getJavaType()).isInstance(value)) {
      throw new IllegalArgumentException(String
          .format("Value class %s does not match required Type class %s",
              value.getClass().getName(),
              Primitives.wrap(type.getJavaType()).getClass().getName()));
    }
    return all;
  }
  ...
  private AllOrNoneValueSet checkCompatibility(
      ValueSet other) {
    ...
    if (!(other instanceof AllOrNoneValueSet)) {
      throw new IllegalArgumentException(
          String.format("ValueSet is not a AllOrNoneValueSet: %s", other.getClass()));
    }
