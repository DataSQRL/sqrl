...
import static ai.dataeng.sqml.common.type.Decimals.longTenToNth;
 ...
 * <p>
 * 127_bit_unscaled_decimal_value is stored in little endian format to provide easy conversion
 * to/from long and int types.
 * <p>
...
public final class UnscaledDecimal128Arithmetic {
  ...
  static {
    ...
    if (!ByteOrder.nativeOrder().equals(ByteOrder.LITTLE_ENDIAN)) {
      throw new IllegalStateException(
          "UnsignedDecimal128Arithmetic is supported on little-endian machines only");
    }
  }
  ...
  /**
   * Instead of throwing overflow exception, this function returns: 0 when there was no overflow +1
   * when there was overflow -1 when there was underflow
   */
   ...
   */
  private static long addUnsignedReturnOverflow(Slice left, Slice right, Slice result,
   ...
   */
  private static void subtractUnsigned(Slice left, Slice right, Slice result,
      boolean resultNegative) {
    // TODO: consider two 7 bytes operations
    int l0 = getInt(left, 0);
    int l1 = getInt(left, 1);
    ...
    if ((intermediateResult >> 32) != 0) {
      throw new IllegalStateException(
          format("Non empty carry over after subtracting [%d]. right > left?",
  ...
  public static int compare(long leftRawLow, long leftRawHigh, long rightRawLow,
      long rightRawHigh) {
    ...
    if (leftStrictlyNegative != rightStrictlyNegative) {
      ...
      return compareUnsigned(leftRawLow, leftHigh, rightRawLow, rightHigh) * (leftStrictlyNegative
          ? -1 : 1);
    }
  }
  ...
  /**
   * Scale down the value for 10**tenScale (this := this / 5**tenScale). This method rounds-up, eg
   * 44/10=4, 44/10=5.
   */
  ...
  private static Slice divideRoundUp(long dividendLow, long dividendHigh, int dividendScaleFactor,
      long divisorLow, long divisorHigh) {
    Slice quotient = unscaledDecimal();
    Slice remainder = unscaledDecimal();
    divide(dividendLow, dividendHigh, dividendScaleFactor, divisorLow, divisorHigh, 0, quotient,
        remainder);
  ...
  public static Slice remainder(long dividend, int dividendScaleFactor, long divisor,
  ...
  public static Slice remainder(long dividend, int dividendScaleFactor, Slice divisor,
      int divisorScaleFactor) {
    return remainder(
        Math.abs(dividend), dividend < 0 ? SIGN_LONG_MASK : 0, dividendScaleFactor,
  ...
  public static Slice remainder(Slice dividend, int dividendScaleFactor, long divisor,
      int divisorScaleFactor) {
    return remainder(
        getRawLong(dividend, 0), getRawLong(dividend, 1), dividendScaleFactor,
        Math.abs(divisor), divisor < 0 ? SIGN_LONG_MASK : 0, divisorScaleFactor);
  ...
  public static Slice remainder(Slice dividend, int dividendScaleFactor, Slice divisor,
      int divisorScaleFactor) {
    return remainder(
        getRawLong(dividend, 0), getRawLong(dividend, 1), dividendScaleFactor,
        getRawLong(divisor, 0), getRawLong(divisor, 1), divisorScaleFactor);
  ...
  private static Slice remainder(long dividendLow, long dividendHigh, int dividendScaleFactor,
      long divisorLow, long divisorHigh, int divisorScaleFactor) {
    Slice quotient = unscaledDecimal();
    Slice remainder = unscaledDecimal();
    divide(dividendLow, dividendHigh, dividendScaleFactor, divisorLow, divisorHigh,
        divisorScaleFactor, quotient, remainder);
    return remainder;
  }

  // visible for testing
  static void divide(Slice dividend, int dividendScaleFactor, Slice divisor, int divisorScaleFactor,
      Slice quotient, Slice remainder) {
    divide(getRawLong(dividend, 0), getRawLong(dividend, 1), dividendScaleFactor,
        getRawLong(divisor, 0), getRawLong(divisor, 1), divisorScaleFactor, quotient, remainder);
  }

  private static void divide(long dividendLow, long dividendHigh, int dividendScaleFactor,
      long divisorLow, long divisorHigh, int divisorScaleFactor, Slice quotient, Slice remainder) {
  ...
  /**
   * Divides mutableDividend / mutable divisor Places quotient in first argument and reminder in
   * first argument
   */
  ...
  private static void divideKnuthNormalized(int[] remainder, int dividendLength, int[] divisor,
      int divisorLength, int[] quotient) {
    ...
    for (int reminderIndex = dividendLength - 1; reminderIndex >= divisorLength; reminderIndex--) {
      int qHat = estimateQuotient(remainder[reminderIndex], remainder[reminderIndex - 1],
          remainder[reminderIndex - 2], v1, v0);
      if (qHat != 0) {
        boolean overflow = multiplyAndSubtractUnsignedMultiPrecision(remainder, reminderIndex,
            divisor, divisorLength, qHat);
  ...
  private static int estimateQuotient(int u2, int u1, int u0, int v1, int v0) {
    ...
    while (Long.compareUnsigned(rhat, INT_BASE) < 0
        && Long.compareUnsigned((v0 & LONG_MASK) * qhat, combineInts(lowInt(rhat), u0)) > 0) {
      iterations++;
      qhat--;
      rhat += (v1 & LONG_MASK);
    }
  ...
  /**
   * Calculate multi-precision [left - right * multiplier] with given left offset and length. Return
   * true when overflow occurred
   */
  private static boolean multiplyAndSubtractUnsignedMultiPrecision(int[] left, int leftOffset,
      int[] right, int length, int multiplier) {
    ...
    for (int rightIndex = 0; rightIndex < length; rightIndex++, leftIndex++) {
      multiplyAccumulator =
          (right[rightIndex] & LONG_MASK) * unsignedMultiplier + multiplyAccumulator;
      subtractAccumulator =
          (subtractAccumulator + (left[leftIndex] & LONG_MASK)) - (lowInt(multiplyAccumulator)
              & LONG_MASK);
  ...
  private static void addUnsignedMultiPrecision(int[] left, int leftOffset, int[] right,
      int length) {
    int leftIndex = leftOffset - length;
    int carry = 0;
    for (int rightIndex = 0; rightIndex < length; rightIndex++, leftIndex++) {
      long accumulator =
          (left[leftIndex] & LONG_MASK) + (right[rightIndex] & LONG_MASK) + (carry & LONG_MASK);
  ...
  private static void setNegative(Slice decimal, boolean negative) {
    setRawInt(decimal, SIGN_INT_INDEX,
        getInt(decimal, SIGN_INT_INDEX) | (negative ? SIGN_INT_MASK : 0));
  }
