...
public final class TimeZoneKey {
  ...
  private static final TimeZoneKey[] OFFSET_TIME_ZONE_KEYS = new TimeZoneKey[
      OFFSET_TIME_ZONE_MAX - OFFSET_TIME_ZONE_MIN + 1];
  ...
  public static TimeZoneKey getTimeZoneKey(short timeZoneKey) {
    checkArgument(timeZoneKey < TIME_ZONE_KEYS.length && TIME_ZONE_KEYS[timeZoneKey] != null,
  ...
  public static TimeZoneKey getTimeZoneKeyForOffset(long offsetMinutes) {
    ...
    if (!(offsetMinutes >= OFFSET_TIME_ZONE_MIN && offsetMinutes <= OFFSET_TIME_ZONE_MAX)) {
      throw new RuntimeException(
          format("Invalid offset minutes %s", offsetMinutes));
    }
