...
import static ai.dataeng.sqml.common.type.TypeUtils.normalizeEnumMap;
import static ai.dataeng.sqml.common.type.TypeUtils.validateEnumMap;
...
public class VarcharEnumType
  ...
  public VarcharEnumType(VarcharEnumMap enumMap) {
    super(VarcharType.UNBOUNDED_LENGTH, new TypeSignature(
        StandardTypes.VARCHAR_ENUM,
        TypeSignatureParameter.of(enumMap)));
  ...
  public static class VarcharEnumMap {
    ...
    @JsonCreator
    public VarcharEnumMap(@JsonProperty("typeName") String typeName,
        @JsonProperty("enumMap") Map<String, String> enumMap) {
    ...
    public String toString() {
      ...
      return format("%s{%s}", typeName, enumMap.entrySet().stream()
          ...
          .map(e -> format("\"%s\": \"%s\"", e.getKey().replaceAll("\"", "\"\""),
              base32.encodeAsString(e.getValue().getBytes())))
