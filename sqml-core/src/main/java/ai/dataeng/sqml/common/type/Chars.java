...
import static ai.dataeng.sqml.common.type.Varchars.byteCount;
import static ai.dataeng.sqml.common.type.Varchars.truncateToLength;
...
public final class Chars {
  ...
  public static Slice padSpaces(Slice slice, Type type) {
    ...
    return padSpaces(slice, (CharType) type);
  }
  ...
  public static Slice truncateToLengthAndTrimSpaces(Slice slice,
      Type type) {
    requireNonNull(type, "type is null");
    if (!isCharType(type)) {
      throw new IllegalArgumentException("type must be the instance of CharType");
    }
    return truncateToLengthAndTrimSpaces(slice,
        (CharType) type);
  }
