...
public class TypeSignatureBase {
  ...
  private TypeSignatureBase(String standardTypeBase) {
    ...
    checkArgument(standardTypeBase.chars().noneMatch(c -> c == '.'),
        "Standard type %s should not have '.' in it", standardTypeBase);
    checkArgument(validateName(standardTypeBase), "Bad characters in base type: %s",
        standardTypeBase);
  ...
  private TypeSignatureBase(QualifiedObjectName typeName) {
    ...
    checkArgument(validateName(typeName.getObjectName()), "Bad characters in typeName: %s",
        typeName);
  ...
  private TypeSignatureBase(QualifiedObjectName typeName, String standardTypeBase) {
    checkArgument(typeName != null && standardTypeBase != null,
        "typeName or standardTypeBase is null");
    checkArgument(validateName(typeName.getObjectName()), "Bad characters in type name: %s",
        typeName.getObjectName());
    checkArgument(validateName(standardTypeBase), "Bad characters in base type: %s",
        standardTypeBase);
    this.standardTypeBase = Optional.of(standardTypeBase);
    this.typeName = Optional.of(typeName);
  ...
  public static TypeSignatureBase of(String name) {
    ...
    if (pos >= 0) {
      return new TypeSignatureBase(QualifiedObjectName.valueOf(name.substring(0, pos)),
  ...
  public static TypeSignatureBase of(
      UserDefinedType userDefinedType) {
    return new TypeSignatureBase(userDefinedType.getUserDefinedTypeName(),
        userDefinedType.getPhysicalTypeSignature().getTypeSignatureBase().getStandardTypeBase());
  }
  ...
  public String getStandardTypeBase() {
    checkArgument(standardTypeBase.isPresent(),
        "TypeSignatureBase %s does not have standard type base", toString());
  ...
  public boolean equals(Object obj) {
    ...
    return Objects.equals(standardTypeBase.map(s -> s.toLowerCase(ENGLISH)),
        other.standardTypeBase.map(s -> s.toLowerCase(ENGLISH))) &&
