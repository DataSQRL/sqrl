...
import static ai.dataeng.sqml.common.type.Decimals.MAX_SHORT_PRECISION;
...
public abstract class DecimalType
  ...
  DecimalType(int precision, int scale, Class<?> javaType) {
    super(new TypeSignature(StandardTypes.DECIMAL,
        buildTypeParameters(precision, scale)), javaType);
  ...
  void validatePrecisionScale(int precision, int scale, int maxPrecision) {
    if (precision <= 0 || precision > maxPrecision) {
      throw new RuntimeException(
          "DECIMAL precision must be in range [1, " + MAX_PRECISION + "]");
    }

    if (scale < 0 || scale > precision) {
      throw new RuntimeException("DECIMAL scale must be in range [0, precision]");
    }
  }
}
