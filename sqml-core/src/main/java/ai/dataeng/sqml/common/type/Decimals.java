...
import static ai.dataeng.sqml.common.type.DecimalType.createDecimalType;
import static ai.dataeng.sqml.common.type.UnscaledDecimal128Arithmetic.toUnscaledString;
import static ai.dataeng.sqml.common.type.UnscaledDecimal128Arithmetic.unscaledDecimal;
import static ai.dataeng.sqml.common.type.UnscaledDecimal128Arithmetic.unscaledDecimalToBigInteger;
...
public final class Decimals {
  ...
  // visible for testing
  public static DecimalParseResult parseIncludeLeadingZerosInPrecision(
      String stringValue) {
    return parse(stringValue, true);
  }

  private static DecimalParseResult parse(String stringValue,
      boolean includeLeadingZerosInPrecision) {
    ...
    return new DecimalParseResult(value,
  ...
  /**
   * Converts {@link BigDecimal} to {@link Slice} representing it for long {@link
   * DecimalType}. It is caller responsibility to ensure that {@code
   * value.scale()} equals to {@link DecimalType#getScale()}.
   */
  ...
  public static boolean overflows(long value, int precision) {
    if (precision > MAX_SHORT_PRECISION) {
      throw new IllegalArgumentException(
          "expected precision to be less than " + MAX_SHORT_PRECISION);
    }
  ...
  public static boolean overflows(BigInteger value) {
    return value.compareTo(MAX_DECIMAL_UNSCALED_VALUE) > 0
        || value.compareTo(MIN_DECIMAL_UNSCALED_VALUE) < 0;
  }
  ...
  public static BigDecimal rescale(BigDecimal value,
      DecimalType type) {
