...
import ai.dataeng.sqml.io.sources.util.TimeAnnotatedRecord;
...
//            if (filesource.getConfiguration().isDiscoverFiles()) monitorDuration = Duration.ofSeconds(10);
            FileEnumeratorProvider fileEnumerator = new FileEnumeratorProvider(filesource, source.getCanonicalizer(), tblConfig);
            ...
            if (parser instanceof TextLineFormat.Parser) {
                org.apache.flink.connector.file.src.FileSource.FileSourceBuilder<String> builder =
                        org.apache.flink.connector.file.src.FileSource.forRecordStreamFormat(
                                new org.apache.flink.connector.file.src.reader.TextLineFormat(sourceConfig.getCharset()), FilePath.toFlinkPath(filesource.getPath()));
...
//              stream.assignTimestampsAndWatermarks(WatermarkStrategy.<SourceRecord>forMonotonousTimestamps().withTimestampAssigner((event, timestamp) -> event.getSourceTime().toEpochSecond()));
            ...
            if (parser instanceof TextLineFormat.Parser) {
                ...
                builder.setDeserializer(new KafkaTimeValueDeserializationSchemaWrapper<>(new SimpleStringSchema()));

                DataStreamSource<TimeAnnotatedRecord<String>> textSource = env.fromSource(builder.build(), WatermarkStrategy.noWatermarks(), flinkSourceName);
    ...
    public static DataStream<Format.Parser.Result> parseStream(DataStreamSource<String> textSource, TextLineFormat.Parser textparser) {
        return textSource.map(s -> textparser.parse(s)).filter(Format.Parser.Result::isSuccess);
    }

    public static DataStream<Format.Parser.Result> parseTimeStream(DataStreamSource<TimeAnnotatedRecord<String>> textSource, TextLineFormat.Parser textparser) {
    ...
    public static DataStream<SourceRecord.Raw> parseToRecord(DataStream<Format.Parser.Result> parsedStream) {
    ...
    static class KafkaTimeValueDeserializationSchemaWrapper<T> implements KafkaRecordDeserializationSchema<TimeAnnotatedRecord<T>> {
        ...
        @Override
        public void deserialize(ConsumerRecord<byte[], byte[]> message, Collector<TimeAnnotatedRecord<T>> out)
                throws IOException {
