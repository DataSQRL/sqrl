...
import static ai.dataeng.sqml.analyzer.SemanticExceptions.subQueryNotSupportedError;
import static ai.dataeng.sqml.common.type.BooleanType.BOOLEAN;
import static ai.dataeng.sqml.planner.ExpressionNodeInliner.replaceExpression;
import static ai.dataeng.sqml.planner.optimizations.ApplyNodeUtil.verifySubquerySupported;
import static ai.dataeng.sqml.planner.optimizations.PlanNodeSearcher.searchFrom;
...
import static ai.dataeng.sqml.sql.tree.ComparisonExpression.Operator.EQUAL;
import static ai.dataeng.sqml.sql.util.AstUtils.nodeContains;
...
import ai.dataeng.sqml.plan.AssignmentUtils;
import ai.dataeng.sqml.plan.Assignments;
import ai.dataeng.sqml.plan.EnforceSingleRowNode;
import ai.dataeng.sqml.plan.FilterNode;
import ai.dataeng.sqml.plan.LateralJoinNode;
import ai.dataeng.sqml.plan.PlanNode;
import ai.dataeng.sqml.plan.ProjectNode;
import ai.dataeng.sqml.plan.SimplePlanRewriter;
...
import ai.dataeng.sqml.sql.tree.QuantifiedComparisonExpression.Quantifier;
...
import ai.dataeng.sqml.util.MorePredicates;
...
class SubqueryPlanner {
  ...
  SubqueryPlanner(
      ...
      Metadata metadata,
      Session session, AssignmentContext context) {
   ...
   */
  private static Optional<Expression> tryResolveMissingExpression(PlanBuilder subPlan,
  ...
  private static Set<Expression> extractColumnReferences(Expression expression,
  ...
  public PlanBuilder handleSubqueries(PlanBuilder builder, Collection<Expression> expressions,
      Node node) {
  ...
  public PlanBuilder handleUncorrelatedSubqueries(PlanBuilder builder,
  ...
  private PlanBuilder handleSubqueries(PlanBuilder builder, Expression expression, Node node,
    ...
    builder = appendExistsSubqueryApplyNodes(builder, collectExistsSubqueries(expression, node),
        correlationAllowed);
    builder = appendQuantifiedComparisonApplyNodes(builder,
        collectQuantifiedComparisonSubqueries(expression, node), correlationAllowed, node);
    return builder;
  }
  ...
  public Set<QuantifiedComparisonExpression> collectQuantifiedComparisonSubqueries(
      Expression expression, Node node) {
    return analysis.getQuantifiedComparisonSubqueries(node)
        .stream()
        .filter(
            quantifiedComparison -> nodeContains(expression, quantifiedComparison.getSubquery()))
        .collect(toImmutableSet());
  }

  private PlanBuilder appendInPredicateApplyNodes(PlanBuilder subPlan,
      Set<InPredicate> inPredicates, boolean correlationAllowed, Node node) {
  ...
  private PlanBuilder appendInPredicateApplyNode(PlanBuilder subPlan, InPredicate inPredicate,
      boolean correlationAllowed, Node node) {
    ...
    subPlan = subPlan.appendProjections(ImmutableList.of(inPredicate.getValue()), variableAllocator,
        idAllocator);
    ...
    subqueryPlan = subqueryPlan
        .appendProjections(ImmutableList.of(valueListSubquery), variableAllocator, idAllocator);
    SymbolReference valueList = new SymbolReference(
        subqueryPlan.translate(valueListSubquery).getName());

    VariableReferenceExpression rewrittenValue = subPlan.translate(inPredicate.getValue());
    InPredicate inPredicateSubqueryExpression = new InPredicate(
        new SymbolReference(rewrittenValue.getName()), valueList);
    VariableReferenceExpression inPredicateSubqueryVariable = variableAllocator
        .newVariable(inPredicateSubqueryExpression, BOOLEAN);

    subPlan.getTranslations().put(inPredicate, inPredicateSubqueryVariable);

    return appendApplyNode(subPlan, inPredicate, subqueryPlan.getRoot(), Assignments
            .of(inPredicateSubqueryVariable, castToRowExpression(inPredicateSubqueryExpression)),
        correlationAllowed);
  }

  private PlanBuilder appendScalarSubqueryApplyNodes(PlanBuilder builder,
      Set<SubqueryExpression> scalarSubqueries, boolean correlationAllowed) {
  ...
  private PlanBuilder appendScalarSubqueryApplyNode(PlanBuilder subPlan,
      SubqueryExpression scalarSubquery, boolean correlationAllowed) {
    ...
    subqueryPlan = subqueryPlan
        .withNewRoot(new EnforceSingleRowNode(idAllocator.getNextId(), subqueryPlan.getRoot()));
    subqueryPlan = subqueryPlan.appendProjections(coercions, variableAllocator, idAllocator);

    VariableReferenceExpression uncoercedScalarSubqueryVariable = subqueryPlan
        .translate(uncoercedScalarSubquery);
    ...
    return appendLateralJoin(subPlan, subqueryPlan, scalarSubquery.getQuery(), correlationAllowed,
        LateralJoinNode.Type.LEFT);
  }

  public PlanBuilder appendLateralJoin(PlanBuilder subPlan, PlanBuilder subqueryPlan, Query query,
      boolean correlationAllowed, LateralJoinNode.Type type) {
    ...
    return new PlanBuilder(
        ...
        new LateralJoinNode(
            ...
            subPlan.getRoot(),
            subqueryNode,
            ImmutableList.copyOf(VariablesExtractor
                .extractUnique(correlation.values(), variableAllocator.getTypes())),
            type,
  ...
  private PlanBuilder appendExistsSubqueryApplyNodes(PlanBuilder builder,
      Set<ExistsPredicate> existsPredicates, boolean correlationAllowed) {
  ...
  private PlanBuilder appendExistSubqueryApplyNode(PlanBuilder subPlan,
      ExistsPredicate existsPredicate, boolean correlationAllowed) {
    ...
    PlanNode subqueryNode = new ProjectNode(idAllocator.getNextId(), subqueryPlan.getRoot(),
        Assignments.of());
  ...
  private PlanBuilder appendQuantifiedComparisonApplyNodes(PlanBuilder subPlan,
      Set<QuantifiedComparisonExpression> quantifiedComparisons, boolean correlationAllowed,
      Node node) {
    for (QuantifiedComparisonExpression quantifiedComparison : quantifiedComparisons) {
      subPlan = appendQuantifiedComparisonApplyNode(subPlan, quantifiedComparison,
          correlationAllowed, node);
    }
    return subPlan;
  }

  private PlanBuilder appendQuantifiedComparisonApplyNode(PlanBuilder subPlan,
      QuantifiedComparisonExpression quantifiedComparison, boolean correlationAllowed, Node node) {
    ...
    switch (quantifiedComparison.getOperator()) {
      case EQUAL:
        switch (quantifiedComparison.getQuantifier()) {
          ...
          case SOME:
            // A = ANY B <=> A IN B
            InPredicate inPredicate = new InPredicate(quantifiedComparison.getValue(),
        ...
        switch (quantifiedComparison.getQuantifier()) {
          case ALL:
            ...
            subPlan.getTranslations()
                .put(quantifiedComparison, subPlan.getTranslations().rewrite(notAny));
          ...
          case SOME:
            ...
            subPlan.getTranslations()
                .put(quantifiedComparison, subPlan.getTranslations().rewrite(notAll));
    ...
    throw new IllegalArgumentException(
        format("Unexpected quantified comparison: '%s %s'",
            quantifiedComparison.getOperator().getValue(), quantifiedComparison.getQuantifier()));
  }

  private PlanBuilder planQuantifiedApplyNode(PlanBuilder subPlan,
      QuantifiedComparisonExpression quantifiedComparison, boolean correlationAllowed) {
    subPlan = subPlan
        .appendProjections(ImmutableList.of(quantifiedComparison.getValue()), variableAllocator,
            idAllocator);
    ...
    subqueryPlan = subqueryPlan
        .appendProjections(ImmutableList.of(quantifiedSubquery), variableAllocator, idAllocator);
    ...
    VariableReferenceExpression coercedQuantifiedComparisonVariable = variableAllocator
        .newVariable(coercedQuantifiedComparison, BOOLEAN);
    ...
    return appendApplyNode(
        ...
        Assignments.of(coercedQuantifiedComparisonVariable,
            castToRowExpression(coercedQuantifiedComparison)),
        correlationAllowed);
  }

  /**
   * Implicit coercions are added when mapping an expression to symbol in {@link TranslationMap}.
   * Coercions for expression are obtained from {@link Analysis} by identity comparison. Create a
   * copy of subquery in order to get a subquery expression that does not have any coercion assigned
   * to it {@link Analysis}.
   */
  ...
  private PlanBuilder appendApplyNode(PlanBuilder subPlan, Node subquery, PlanNode subqueryNode,
      Assignments subqueryAssignments, boolean correlationAllowed) {
    Map<Expression, Expression> correlation = extractCorrelation(subPlan, subqueryNode);
    if (!correlationAllowed && !correlation.isEmpty()) {
  ...
  /**
   * @return a set of reference expressions which cannot be resolved within this plan. For plan
   * representing: SELECT a, b FROM (VALUES 1) T(a). It will return a set containing single
   * expression reference to 'b'.
   */
  private Set<Expression> extractOuterColumnReferences(PlanNode planNode) {
    ...
    return ExpressionExtractor.extractExpressions(planNode).stream()
        ...
        .flatMap(expression -> extractColumnReferences(expression, analysis.getColumnReferences())
            .stream())
        .collect(toImmutableSet());
  }

  private PlanNode replaceExpressionsWithSymbols(PlanNode planNode,
      Map<Expression, Expression> mapping) {
    if (mapping.isEmpty()) {
      return planNode;
    }

    return SimplePlanRewriter
        .rewriteWith(new ExpressionReplacer(idAllocator, mapping), planNode, null);
  }

  private static class ColumnReferencesExtractor
      extends DefaultExpressionTraversalVisitor<Void, Builder<Expression>> {
    ...
    protected Void visitDereferenceExpression(DereferenceExpression node,
        ImmutableSet.Builder<Expression> builder) {
  ...
  private static class ExpressionReplacer
    ...
    public ExpressionReplacer(PlanNodeIdAllocator idAllocator,
        Map<Expression, Expression> mapping) {
    ...
    public PlanNode visitProject(ProjectNode node, RewriteContext<Void> context) {
      ...
      Assignments assignments = AssignmentUtils
          .rewrite(rewrittenNode.getAssignments(),
              expression -> replaceExpression(expression, mapping));
    ...
    public PlanNode visitFilter(FilterNode node, RewriteContext<Void> context) {
      ...
      return new FilterNode(idAllocator.getNextId(), rewrittenNode.getSource(), castToRowExpression(
          replaceExpression(castToExpression(rewrittenNode.getPredicate()), mapping)));
    }
    ...
    public PlanNode visitValues(ValuesNode node, RewriteContext<Void> context) {
      ...
      List<List<RowExpression>> rewrittenRows = rewrittenNode.getRows().stream()
          .map(row -> row.stream()
              .map(column -> castToRowExpression(
                  replaceExpression(castToExpression(column), mapping)))
              .collect(toImmutableList()))
          .collect(toImmutableList());
