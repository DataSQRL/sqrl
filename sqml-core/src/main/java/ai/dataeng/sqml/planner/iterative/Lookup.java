...
public interface Lookup {
  ...
  /**
   * A Lookup implementation that does not perform lookup. It satisfies contract by rejecting {@link
   * GroupReference}-s.
   */
  ...
  static Lookup from(Function<GroupReference, Stream<PlanNode>> resolver) {
    return node -> {
      checkArgument(node instanceof GroupReference, "Node '%s' is not a GroupReference",
  ...
  /**
   * Resolves a node by materializing GroupReference nodes representing symbolic references to other
   * nodes. This method is deprecated since is assumes group contains only one node.
   * <p>
  ...
  /**
   * Resolves nodes by materializing GroupReference nodes representing symbolic references to other
   * nodes.
   *
