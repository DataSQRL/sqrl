...
import static ai.dataeng.sqml.analyzer.ExpressionTreeUtils.tryResolveEnumLiteral;
...
class TranslationMap {
  ...
  public void setFieldMappings(List<VariableReferenceExpression> variables) {
    checkArgument(variables.size() == fieldVariables.length,
        "size of variables list (%s) doesn't match number of expected fields (%s)",
        variables.size(), fieldVariables.length);
  ...
  public void put(Expression expression, VariableReferenceExpression variable) {
    if (expression instanceof FieldReference) {
      ...
      expressionToVariables
          .put(new SymbolReference(rewriteBase.getVariable(fieldIndex).getName()), variable);
      return;
    }
  ...
  public VariableReferenceExpression get(Expression expression) {
    ...
    if (!expressionToVariables.containsKey(translated)) {
      checkArgument(expressionToExpressions.containsKey(translated),
          "No mapping for expression: %s", expression);
  ...
  private Expression translateNamesToSymbols(Expression expression) {
    return ExpressionTreeRewriter.rewriteWith(new ExpressionRewriter<Void>() {
      ...
      @Override
      public Expression rewriteFieldReference(FieldReference node, Void context,
          ExpressionTreeRewriter<Void> treeRewriter) {
        VariableReferenceExpression variable = rewriteBase.getVariable(node.getFieldIndex());
        checkState(variable != null, "No variable mapping for node '%s' (%s)", node,
            node.getFieldIndex());
        return new SymbolReference(variable.getName());
      }

      @Override
      public Expression rewriteIdentifier(Identifier node, Void context,
          ExpressionTreeRewriter<Void> treeRewriter) {
      ...
      @Override
      public Expression rewriteDereferenceExpression(DereferenceExpression node, Void context,
          ExpressionTreeRewriter<Void> treeRewriter) {
          ...
          if (resolvedField.isPresent()) {
            if (resolvedField.get().isLocal()) {
              return getVariable(rewriteBase, node)
                  .map(variable -> coerceIfNecessary(node, new SymbolReference(variable.getName())))
                  .orElseThrow(
                      () -> new IllegalStateException("No symbol mapping for node " + node));
            }
          }
      ...
      public Expression rewriteParameter(Parameter node, Void context,
          ExpressionTreeRewriter<Void> treeRewriter) {
        checkState(analysis.getParameters().size() > node.getPosition(),
  ...
  private Optional<VariableReferenceExpression> getVariable(RelationPlan plan,
