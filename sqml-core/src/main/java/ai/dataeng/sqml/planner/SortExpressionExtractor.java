...
import ai.dataeng.sqml.relation.LogicalRowExpressions;
...
import ai.dataeng.sqml.relational.DeterminismEvaluator;
import ai.dataeng.sqml.relational.RowExpressionDeterminismEvaluator;
...
/**
 * Extracts sort expression to be used for creating {@link com.facebook.presto.operator.SortedPositionLinks}
 * from join filter expression. Currently this class can extract sort and search expressions from
 * filter function conjuncts of shape:
 * <p>
 * {@code A.a < f(B.x, B.y, B.z)} or {@code f(B.x, B.y, B.z) < A.a}
 * <p>
 * where {@code a} is the build side symbol reference and {@code x,y,z} are probe side symbol
 * references. Any of inequality operators ({@code <,<=,>,>=}) can be used. Same build side symbol
 * need to be used in all conjuncts.
 */
public final class SortExpressionExtractor {
  ...
  public static Optional<SortExpressionContext> extractSortExpression(
      Set<VariableReferenceExpression> buildVariables, RowExpression filter,
      FunctionAndTypeManager functionAndTypeManager) {
    List<RowExpression> filterConjuncts = LogicalRowExpressions.extractConjuncts(filter);
    SortExpressionVisitor visitor = new SortExpressionVisitor(buildVariables,
    ...
    List<SortExpressionContext> sortExpressionCandidates = filterConjuncts.stream()
        ...
        .map(Optional::get)
        .collect(toMap(SortExpressionContext::getSortExpression, identity(),
            SortExpressionExtractor::merge))
        .values()
        .stream()
  ...
  private static SortExpressionContext merge(SortExpressionContext left,
      SortExpressionContext right) {
  ...
  private static Optional<VariableReferenceExpression> asBuildVariableReference(
      Set<VariableReferenceExpression> buildLayout, RowExpression expression) {
  ...
  private static boolean hasBuildVariableReference(Set<VariableReferenceExpression> buildVariables,
  ...
  private static class SortExpressionVisitor
    ...
    public SortExpressionVisitor(Set<VariableReferenceExpression> buildVariables,
        FunctionAndTypeManager functionAndTypeManager) {
    ...
    public Optional<SortExpressionContext> visitCall(CallExpression call, Void context) {
      ...
      if (!functionMetadata.getOperatorType().map(OperatorType::isComparisonOperator)
          .orElse(false)) {
        return Optional.empty();
      }
      ...
      switch (functionMetadata.getOperatorType().get()) {
        ...
        case LESS_THAN_OR_EQUAL:
          ...
          Optional<VariableReferenceExpression> sortChannel = asBuildVariableReference(
              buildVariables, right);
          ...
          if (sortChannel.isPresent() && !hasBuildReferencesOnOtherSide) {
            return sortChannel.map(variableReference -> new SortExpressionContext(variableReference,
                singletonList(call)));
          }
          return Optional.empty();
        default:
    ...
    @Override
    public Optional<SortExpressionContext> visitVariableReference(
        VariableReferenceExpression reference, Void context) {
      return Optional.empty();
    }
  ...
  private static class BuildVariableReferenceFinder
    ...
    public BuildVariableReferenceFinder(Set<VariableReferenceExpression> buildVariables) {
      this.buildVariables = ImmutableSet
          .copyOf(requireNonNull(buildVariables, "buildVariables is null"));
    }
