 ...
 * Ideally, users should never do type checking or down casting for future convenience of
 * refactoring.
 */
public final class OriginalExpressionUtils {
  ...
  /**
   * OriginalExpression is a RowExpression container holding an {@param Expression} object that
   * cannot be translated directly while constructing PlanNode. Typical examples are cases with
   * `WITH` clauses or alias that the type or arguments information are not directly available given
   * the substrees are not formed yet. OriginalExpression should not exist after optimization or
   * serialized over the wire. All OriginalExpression should be translated to other corresponding
   * RowExpression objects ultimately.
   */
  private static final class OriginalExpression
    ...
    public <R, C> R accept(RowExpressionVisitor<R, C> visitor, C context) {
      throw new UnsupportedOperationException(
          "OriginalExpression cannot appear in a RowExpression tree");
    }
  }
}
