...
class ScopeReferenceExtractor {
  ...
  public static Stream<Expression> getReferencesToScope(Node node, Analysis analysis, Scope scope) {
    Multimap<NodeRef<Expression>, FieldId> columnReferences = analysis
  ...
  private static boolean hasReferenceToScope(Expression node, Scope scope,
      Multimap<NodeRef<Expression>, FieldId> columnReferences) {
    return columnReferences.get(NodeRef.of(node)).stream()
        .anyMatch(fieldId -> isFieldFromScope(fieldId, scope));
  }

  public static boolean isFieldFromScope(FieldId fieldId,
      Scope scope) {
