...
public class ApplyNode
   ...
   * <p>
   * Subquery expressions are different than other expressions in a sense that they might use an
   ...
   * <p>
   * Example 1: - expression: input_symbol_X IN (subquery_symbol_Y) - meaning: if set consisting of
   * all values for subquery_symbol_Y contains value represented by input_symbol_X
   * <p>
   * Example 2: - expression: input_symbol_X < ALL (subquery_symbol_Y) - meaning: if input_symbol_X
   * is smaller than all subquery values represented by subquery_symbol_Y
   * <p>
   */
  ...
  public ApplyNode(
    ...
    super(id);
    checkArgument(input.getOutputVariables().containsAll(correlation),
  ...
  public PlanNode replaceChildren(List<PlanNode> newChildren) {
    ...
    return new ApplyNode(getId(), newChildren.get(0), newChildren.get(1), subqueryAssignments,
        correlation, originSubqueryError);
  }
}
