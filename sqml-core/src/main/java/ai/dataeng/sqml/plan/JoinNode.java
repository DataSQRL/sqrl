...
import static ai.dataeng.sqml.plan.JoinNode.DistributionType.PARTITIONED;
import static ai.dataeng.sqml.plan.JoinNode.Type.INNER;
import static ai.dataeng.sqml.planner.SortExpressionExtractor.extractSortExpression;
...
import ai.dataeng.sqml.planner.SortExpressionContext;
...
public class JoinNode
  ...
  public JoinNode(@JsonProperty("id") PlanNodeId id,
    ...
    Set<VariableReferenceExpression> inputVariables = ImmutableSet
        .<VariableReferenceExpression>builder()
        .addAll(left.getOutputVariables())
        .addAll(right.getOutputVariables())
        .build();
//    checkArgument(new HashSet<>(inputVariables).containsAll(outputVariables),
//        "Left and right join inputs do not contain all output variables");
    checkArgument(!isCrossJoin() || inputVariables.size() == outputVariables.size(),
        "Cross join does not support output variables pruning or reordering");

    checkArgument(!(criteria.isEmpty() && leftHashVariable.isPresent()),
        "Left hash variable is only valid in an equijoin");
    checkArgument(!(criteria.isEmpty() && rightHashVariable.isPresent()),
        "Right hash variable is only valid in an equijoin");

    if (distributionType.isPresent()) {
      // The implementation of full outer join only works if the data is hash partitioned.
      checkArgument(
          !(distributionType.get() == DistributionType.REPLICATED && type.mustPartition()),
  ...
  public Optional<SortExpressionContext> getSortExpressionContext(
    ...
    return filter
        .flatMap(
            filter -> extractSortExpression(ImmutableSet.copyOf(right.getOutputVariables()), filter,
  ...
  public PlanNode replaceChildren(List<PlanNode> newChildren) {
    ...
    return new JoinNode(getId(), type, newChildren.get(0), newChildren.get(1), criteria,
        outputVariables, filter, leftHashVariable, rightHashVariable, distributionType,
        dynamicFilters);
  }
  ...
  public static class EquiJoinClause {
    ...
    @JsonCreator
    public EquiJoinClause(@JsonProperty("left") VariableReferenceExpression left,
        @JsonProperty("right") VariableReferenceExpression right) {
