...
public final class TableScanNode
  ...
  public TableScanNode(
    ...
    this.outputVariables = unmodifiableList(
        requireNonNull(outputVariables, "outputVariables is null"));
    this.assignments = unmodifiableMap(
        new HashMap<>(requireNonNull(assignments, "assignments is null")));
    checkArgument(assignments.keySet().containsAll(outputVariables),
...
//      checkArgument(table.getLayout().isPresent(),
//          "tableLayout must be present when currentConstraint or enforcedConstraint is non-trivial");
    }
  }
  ...
  /**
   * A TupleDomain that represents a predicate that every row this TableScan node produces is
   * guaranteed to satisfy.
   * <p>
   * This guarantee can have different origins. For example, it may be successful predicate push
   * down, or inherent guarantee provided by the underlying data.
   * <p>
   * currentConstraint will only be used in planner. It is not transported to worker thus will be
   * null on worker.
   */
  @Nullable
  ...
  /**
   * A TupleDomain that represents a predicate that has been successfully pushed into this TableScan
   * node. In other words, predicates that were removed from filters above the TableScan node
   * because the TableScan node can guarantee it.
   * <p>
   * This field is used to make sure that predicates which were previously pushed down do not get
   * lost in subsequent refinements of the table layout.
   */
  public TupleDomain<ColumnHandle> getEnforcedConstraint() {
    ...
    checkState(enforcedConstraint != null,
        "enforcedConstraint should only be used in planner. It is not transported to workers.");
