...
public class TableFactory {
  ...
  private Column getOrCreateColumn(Expression expression,
    ...
    if (expression instanceof Identifier) {
      ...
      Column column = new Column(alias.map(Identifier::getNamePath).orElseGet(i::getNamePath).getFirst(),
          null, -1, null, 0, List.of(),
    ...
    Column column = new Column(alias.map(Identifier::getNamePath)
        .orElseGet(()->Name.system("unnamedColumn").toNamePath()).getFirst(),
        null, -1, null, 0, List.of(),
        false, false, Optional.empty(), false);
