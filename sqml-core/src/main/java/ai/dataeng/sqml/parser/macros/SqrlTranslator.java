...
import static ai.dataeng.sqml.parser.macros.SqlNodeUtils.and;
import static ai.dataeng.sqml.parser.macros.SqlNodeUtils.eq;
import static ai.dataeng.sqml.parser.macros.SqlNodeUtils.getSiblingCall;
import static ai.dataeng.sqml.parser.macros.SqlNodeUtils.getTableForCall;
import static ai.dataeng.sqml.parser.macros.SqlNodeUtils.ident;
import static ai.dataeng.sqml.parser.macros.SqlNodeUtils.join;
import static ai.dataeng.sqml.parser.macros.SqlNodeUtils.toCall;
import static ai.dataeng.sqml.parser.macros.SqlNodeUtils.unwrapCall;
...
public class SqrlTranslator extends BaseSqrlTranslator {
  ...
  public Scope visitSubquery(SqlBasicCall subquery, Scope scope) {
    new SqrlTranslator(validator, validatorProvider, contextTable)
...
//  }
  ...
  private SqlNode rewriteHaving(SqlNode having, Scope fromScope) {
    if (having != null) {
      return having.accept(new AliasRewriter(mapper));
    }
    return having;
  }
  ...
  private SqlNodeList rewriteGroupBy(SqlNodeList group, SqlSelect query,
    ...
    if (((SqrlValidator)validator).hasAgg(query.getSelectList())) {
      ...
      return (SqlNodeList)new SqlNodeList(nodes, SqlParserPos.ZERO).accept(new AliasRewriter(mapper));
    }

    if (group != null) {
      return (SqlNodeList)group.accept(new AliasRewriter(mapper));
    }
    return group;
  }
  ...
  private SqlNode rewriteWhere(SqlNode where, Scope fromScope) {
    ...
    return where.accept(new AliasRewriter(this.mapper));
  }
  ...
  public SqlNodeList rewriteSelectItems(SqlSelect query, Scope scope) {
    SqlNodeList list = (SqlNodeList)query.getSelectList().accept(new AliasRewriter(this.mapper));
