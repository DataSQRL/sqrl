...
@AllArgsConstructor
public class HKVSRegistryPersistence implements DatasetRegistryPersistence {
    ...
    private final HierarchyKeyValueStore store;
    ...
    public Set<DataSourceConfiguration> getDatasets() {
        return store.getSubKeys(STORE_DATASET_KEY).stream().map(dsName -> {
            ...
            Preconditions.checkArgument(config!=null && !config.validateAndInitialize().isFatal(), "Persistence of configuration failed.");
            Preconditions.checkArgument(name2Key(config.getDatasetName()).equals(dsName));
            return config;
    ...
    @Override
    public void putDataset(DataSourceConfiguration datasource) {
        store.put(datasource,STORE_DATASET_KEY,name2Key(datasource.getDatasetName()),STORE_SOURCE_CONFIG_KEY);
    }

    @Override
    public Set<SourceTableConfiguration> getTables(DataSourceConfiguration source) {
        return store.getSubKeys(STORE_DATASET_KEY,name2Key(source.getDatasetName()),STORE_TABLE_KEY).stream().map(tbName -> {
            SourceTableConfiguration config = store.get(SourceTableConfiguration.class,
                    STORE_DATASET_KEY,name2Key(source.getDatasetName()),STORE_TABLE_KEY,tbName,STORE_TABLE_CONFIG_KEY);
