...
public class GenerateNutData {
    ...
    public static void main(String[] args) throws Exception{
        if(args.length != 4){
            throw new IllegalArgumentException("Expected arguments: [Nut Filename] [Num Customers] [Num Days] [Orders Per Day]");
        }
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(nutFile))) {
            for (String line = reader.readLine(); line!=null; line = reader.readLine()) {
                ...
                if (match) {
                    ...
                    int variants = 0;
                    if (rand.nextDouble() > 0.5) variants = rand.nextInt(other_sizes.length);
        ...
        for (int days = 0; days < numDays ; days++) {
            ...
            for (int i = 0; i < numOrders; i++) {
                int numEntries = Math.min(Math.round(1 + (int)Math.abs(rand.nextGaussian()*numEntriesStdDev)),
                                            products.size()/8);
                ...
                for (Product p : selectProducts) {
                    ...
                    if (rand.nextDouble()<discountLikelihood) {
                        double percentage = minDiscountPercentage + rand.nextDouble()*(maxDiscountPercentage-minDiscountPercentage);
