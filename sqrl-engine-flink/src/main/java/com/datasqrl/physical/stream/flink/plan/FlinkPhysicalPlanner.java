...
public class FlinkPhysicalPlanner {
  ...
  public FlinkStreamPhysicalPlan createStreamGraph(List<? extends OptimizedDAG.Query> streamQueries) {
      ...
      if (query.getSink() instanceof  OptimizedDAG.DatabaseSink) {
        ...
        if (dbConnection instanceof JDBCConnectionProvider) {
          ...
          sinkDescriptor = TableDescriptor.forConnector("jdbc")
                  ...
                  .build();
        } else throw new UnsupportedOperationException("Database not yet supported: " + dbConnection);
      } else {
