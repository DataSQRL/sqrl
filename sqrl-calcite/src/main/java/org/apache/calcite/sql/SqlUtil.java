 ...
 *
 * <p>SQRL modifications: Use a different SqlOperatorTable, use namematcher
 */
public abstract class SqlUtil {
   ...
   *
   * <p>This method takes an array of arguments, since pairwise concatenation means too much string
   * copying.
   *
  ...
  public static boolean matchRoutinesByParameterCount(
    ...
    // start with all routines matching by name
    Iterator<SqlOperator> routines =
        lookupSubjectRoutinesByName(opTab, funcName, SqlSyntax.FUNCTION, category, nameMatcher);
        ...
        Iterators.filter(
            ...
            function -> {
              ...
              if (argNames != null) {
                ...
                permutedArgTypes =
                    permuteArgTypes(
                        paramNames, argNames, argTypes, SqlNameMatchers.withCaseSensitive(false));
  ...
  /**
   * Filters an iterator of routines, keeping only those with the best match for the actual argument
   * types.
   *
   ...
   *
   * @param op operator
   * @param typeList list of types to use for operands. Types may be represented as {@link String},
   *     {@link SqlTypeFamily}, or any object with a valid {@link Object#toString()} method.
  ...
  /**
   * Returns whether a {@link SqlNode node} is a {@link SqlCall call} to a given {@link SqlOperator
   * operator}.
   */
   ...
   *
   * <p>The type inherits the NlsString's {@link Charset} and {@link SqlCollation}, if they are set,
   * otherwise it gets the system defaults.
   *
   * @param typeFactory Type factory
   * @param str String
  ...
  /**
   * If a node is "AS", returns the underlying expression; otherwise returns the node. Returns null
   * if and only if the node is null.
   */
   ...
   *
   * <p>The first element of the list is {@code root}, and the last is the node that matched {@code
   * predicate}. Throws if no node matches.
   */
  ...
  /**
   * Returns an immutable list of {@link RelHint} from sql hints, with a given inherit path from the
   * root node.
   *
   ...
   * @return A copy of {@code rel} if there are any hints can be attached given the hint strategies,
   *     or the original node if such hints don't exist
   */
   ...
   * through to the base class, {@link org.apache.calcite.util.BarfingInvocationHandler}, which will
   * throw an error.
   */
  ...
  /**
   * Walks over a {@link org.apache.calcite.sql.SqlNode} tree and returns the ancestry stack when it
   * finds a given node.
   */
