...
public interface LogFactory {
   ...
   * @param primaryKey The primary key for messages in the log. The primary key columns must be part
   *     of the schema.
   * @param timestamp The timestamp for the messages in the log.
   * @return
   */
  Log create(
      String logId, Name logName, RelDataType schema, List<String> primaryKey, Timestamp timestamp);

  String getEngineName();

  enum TimestampType {
    /** Don't set a timestamp on the created log. Timestamp name is ignored. */
    NONE,
    /**
     * Use the insertion time from the log. This timestamp will be given the provided name. It must
     * be unique.
     */
    LOG_TIME,
