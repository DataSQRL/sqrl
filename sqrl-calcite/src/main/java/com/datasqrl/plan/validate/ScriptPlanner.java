...
public class ScriptPlanner implements StatementVisitor<Void, Void> {
  ...
  public Void visit(SqrlCreateDefinition statement, Void context) {
    var namePath = NamePath.of(statement.getName().names.toArray(String[]::new));
    var logId = statement.getName().names.get(0);

    var typeFactory = TypeFactory.getTypeFactory();
    var fieldBuilder = CalciteUtil.getRelTypeBuilder(typeFactory);
    ...
    var relDataType = fieldBuilder.build();
  ...
  private Log createAndRegisterLogEntity(String logId, NamePath namePath, RelDataType relDataType) {
    var sinkLog = logManager.create(logId, namePath.getLast(),
    ...
    var namespaceObject = createTableResolver.create(
  ...
  public Void visit(SqrlImportDefinition node, Void context) {
    ...
    if (path.getFirst().getDisplay().equals("log")) {
      var log = logManager.getLogs().get(path.getLast().getDisplay());
      var namespaceObject = createTableResolver.create(log.getSource());
      ...
      if (module == null) {
        throw addError(ErrorLabel.GENERIC, node, "Could not find module [%s] at path: [%s]",
      ...
      if (node.getImportPath().isStar()) {
        ...
        namespaceObject.map(object -> object.apply(this, Optional.of(node.getAlias().map(a -> a.names.get(0))
            ...
            .orElseThrow(() -> addError(ErrorLabel.GENERIC, node, "Object [%s] not found in module: %s", path.getLast(), path));
      }
    }
  ...
  public Void visit(SqrlExportDefinition node, Void context) {
    ...
    var path = nameUtil.toNamePath(node.getIdentifier().names);

    var planner = framework.getQueryPlanner();
    var table = planner.getCatalogReader().getTableFromPath(path);
    ...
    var numSelects = modTable.getNumSelects();
    var exportRelNode = planner.getRelBuilder().scan(modTable.getNameId()).build();

    var builtInSink = SystemBuiltInConnectors.forExport(sinkPath.getFirst());
    TableSink tableSink = null;
    if (builtInSink.isPresent()) {
      var connector = builtInSink.get();
      if (connector == SystemBuiltInConnectors.LOGGER) {
        var logger = packageJson.getCompilerConfig().getLogger();
        ...
        var sinkLog = logManager.getLogs().get(sinkPath.getLast().getDisplay());
        if (sinkLog == null) {
      ...
      Optional<TableSink> externalSink = moduleLoader.getModule(sinkPath.popLast())
          ...
          .filter(TableSinkObject.class::isInstance)
    ...
    var resolvedExport = new ResolvedExport(modTable.getNameId(), exportRelNode, numSelects, tableSink);
  ...
  public Void visit(SqrlAssignment assignment, Void context) {
    if (assignment.getIdentifier().names.size() > 1) {
      var path = nameUtil.toNamePath(assignment.getIdentifier().names).popLast();
      var tableFunction = framework.getQueryPlanner().getSchema()
      ...
      var function = Iterables.getOnlyElement(tableFunction);
  ...
  public Void visit(SqrlExpressionQuery node, Void context) {
    ...
    var names = nameUtil.toNamePath(node.getIdentifier().names).popLast();
    var table = resolveModifiableTable(node, names);
...
//    RelOptTable table = planner.getCatalogReader().getTableFromPath(path);
    var rexNode = framework.getQueryPlanner().planExpression(node.getExpression(), table.get().getRowType());
  ...
  public Void visit(SqrlSqlQuery node, Void context) {
    var materializeSelf = materializeSelfQuery(node);
  ...
  public Void postvisit(SqrlAssignment assignment, Void context) {
    ...
    var planner = framework.getQueryPlanner();
    var node = getPreprocessSql().get(assignment);
    boolean materializeSelf = getIsMaterializeTable().get(assignment);
    var parentPath = nameUtil.getParentPath(assignment);
    var normalizeTablePath = new NormalizeTablePath(planner.getCatalogReader(),
        getParamMapping(), new SqlNameUtil(planner.getFramework().getNameCanonicalizer()), errorCollector);
    var sqrlToSql = new SqrlToSql(planner, planner.getCatalogReader(), planner.getOperatorTable(),
        normalizeTablePath, getParameters().get(assignment), framework.getSchema().getUniquePkId(), nameUtil);
    var result = sqrlToSql.rewrite(node, materializeSelf, parentPath);
    this.isA.putAll(sqrlToSql.getIsA());
    validateTopLevelNamed(result.getSqlNode());

    var relNode = planner.plan(Dialect.CALCITE, result.getSqlNode());
    var expanded = planner.expandMacros(relNode);

    var isA = getIsA().get(node);

    if (assignment.getTableArgs().isEmpty()) {
      var path = nameUtil.toNamePath(assignment.getIdentifier().names);
  ...
  public void validateTable(SqrlAssignment statement, SqlNode query,
    ...
    var argResult = transformArgs(parentTable, query, materializeSelf,
    ...
    if (statement instanceof SqrlExpressionQuery) {
      var aggregate = ((SqrlSqlValidator) validator).getAggregate((SqlSelect) query);
      if (aggregate != null) {
    ...
    return SqlNodeVisitor.accept(new SqlTopLevelRelationVisitor<Boolean, Object>() {
      ...
      public Boolean visitSetOperation(SqlCall node, Object context) {
        for (SqlNode operator : node.getOperandList()) {
          var isValid = validateTopLevelNamed(operator);
  ...
  private void validateHasNestedSelf(SqlNode query) {
    ...
    while (lhs instanceof SqlJoin) {
      var join = (SqlJoin) lhs;
      lhs = join.getLeft();
      lhs = stripAs(lhs);
    }
    if (!(lhs instanceof SqlIdentifier identifier)) {
  ...
  public static boolean isSelfTable(SqlNode sqlNode) {
    if (sqlNode instanceof SqlCall &&
      ...
      var id = ((SqlIdentifier) ((SqlCall) sqlNode).getOperandList()
          .get(0));
    ...
    return new SqlShuttle() {
      ...
      public SqlNode visit(SqlIdentifier id) {
        if (isSelfField(nameUtil.toNamePath(id.names)) && !materializeSelf) {
          ...
          var name = id.names.get(1);
          for (FunctionParameter p : parameterList) {
            var s = (SqrlFunctionParameter) p;
          ...
          var functionParameter = new SqrlFunctionParameter(name,
          ...
          var param = new SqlDynamicParam(functionParameter.getOrdinal(),
          ...
          var param = defs.get(0);

          if (paramMapping.get(param) != null) {
            return paramMapping.get(param);
          }

          var index = param.getOrdinal();

          var p = new SqlDynamicParam(index, id.getParserPosition());
  ...
  public Void visit(SqrlJoinQuery node, Void context) {
    visit((SqrlAssignment) node, null);
    var materializeSelf = node.getQuery().getFetch() != null;
    isMaterializeTable.put(node, materializeSelf);
    checkAssignable(node);
    var path = nameUtil.toNamePath(node.getIdentifier().names);
    var parent = path.popLast();
    if (parent.isEmpty()) {
     ...
     */
    var lastAlias = extractLastAlias(node.getQuery().getFrom());
    ...
    var select = new SqlSelectBuilder(node.getQuery())
  ...
  private void checkAssignable(SqrlAssignment node) {
    var path = nameUtil.toNamePath(node.getIdentifier().names).popLast();
    if (path.isEmpty()) {
      return;
    }

    var tableFunction = framework.getQueryPlanner().getSchema().getFunctions(path.getDisplay(), false);
    if (tableFunction.isEmpty()) {
  ...
  private int addColumn(RexNode node, String cName, RelOptTable table) {
    if (table.unwrap(ModifiableTable.class) != null) {
      var table1 = (ModifiableTable) table.unwrap(Table.class);
  ...
  private Optional<RelOptTable> resolveModifiableTable(SqlNode node, NamePath names) {
    var table = framework.getCatalogReader().getTableFromPath(names);
    if (table.isEmpty()) {
  ...
  public RuntimeException addError(ErrorLabel errorCode, CalciteContextException e) {
    var exception = CheckUtil.createAstException(Optional.of(e), errorCode,
  ...
  public static RuntimeException addError(ErrorCollector errorCollector, ErrorLabel errorCode,
    ...
    var exception = CheckUtil.createAstException(errorCode, node,
  ...
  public RuntimeException addError(Throwable cause, ErrorLabel errorCode, SqlNode node,
      String message, Object... format) {
    var exception = CheckUtil.createAstException(Optional.of(cause), errorCode,
  ...
  public void plan(MainScript mainScript, ModuleLoader composite) {
    ...
    for (SqlNode statement : scriptNode.getStatements()) {
      try {
        ...
        var statementErrors = scriptErrors
            .atFile(SqrlAstException.toLocation(statement.getParserPosition()));
