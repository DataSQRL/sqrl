...
public class ScriptPlanner implements StatementVisitor<Void, Void> {
  ...
  private final ArrayListMultimap<SqlNode, FunctionParameter> parameters =
  ...
  public Void visit(SqrlCreateDefinition statement, Void context) {
    ...
    for (SqrlColumnDefinition definition : statement.getColumns()) {
      fieldBuilder.add(
          definition.getColumnName().getSimple(),
  ...
  private Log createAndRegisterLogEntity(String logId, NamePath namePath, RelDataType relDataType) {
    Log sinkLog =
        logManager.create(
            ...
            relDataType,
            List.of("_uuid"),
            new Timestamp("event_time", TimestampType.LOG_TIME));
  ...
  public Void visit(SqrlImportDefinition node, Void context) {
    ...
    if (path.getFirst().getDisplay().equals("log")) {
      ...
      namespaceObject.apply(
          this, node.getAlias().map(SqlIdentifier::getSimple), framework, errorCollector);
    } else {
      ...
      if (node.getImportPath().isStar()) {
        ...
        module
            .getNamespaceObjects()
            .forEach(obj -> obj.apply(this, Optional.empty(), framework, errorCollector));
      } else {
                ...
                object ->
                    object.apply(
                        ...
                        Optional.of(
                            node.getAlias()
                                .map(a -> a.names.get(0))
                                .orElse(/*retain alias*/ path.getLast().getDisplay())),
                        framework,
                        errorCollector))
            .orElseThrow(
                () ->
  ...
  public Void visit(SqrlExportDefinition node, Void context) {
    ...
    Preconditions.checkState(
        table.isPresent(), "Could not find export table: %s", path.getDisplay());
    ...
    Optional<SystemBuiltInConnectors> builtInSink =
        SystemBuiltInConnectors.forExport(sinkPath.getFirst());
    ...
    if (builtInSink.isPresent()) {
      ...
      if (connector == SystemBuiltInConnectors.LOGGER) {
        ...
        if (logger.equalsIgnoreCase("none")) {
          ...
          Log sinkLog =
              logManager.create(
                  modTable.getNameId(),
                  sinkPath.getLast(),
                  exportRelNode.getRowType(),
                  List.of(),
                  LogFactory.Timestamp.NONE);
        ...
        if (sinkLog == null) {
          sinkLog =
              createAndRegisterLogEntity(
                  sinkPath.getLast().getDisplay(), sinkPath, exportRelNode.getRowType());
        }
        tableSink = sinkLog.getSink();
      }
      if (tableSink == null) {
                        ...
                        t.createSourceAndSink(
                            new ConnectorFactoryContext(
                                ...
                                Map.of(
                                    ...
                                    modTable.getNameId(),
                                    "name",
                                    sinkPath.getLast().getDisplay()))))
                .map(t -> TableSinkImpl.create(t, sinkPath.popLast(), Optional.empty()))
                .get();
      }
    } else {
      Optional<TableSink> externalSink =
          moduleLoader
      ...
      if (externalSink.isEmpty()) {
        addError(
            ErrorCode.CANNOT_RESOLVE_TABLESINK,
            node,
            "Cannot resolve table sink: %s",
    ...
    ResolvedExport resolvedExport =
        new ResolvedExport(modTable.getNameId(), exportRelNode, numSelects, tableSink);
  ...
  public Void visit(SqrlAssignment assignment, Void context) {
    if (assignment.getIdentifier().names.size() > 1) {
      ...
      Collection<Function> tableFunction =
          framework.getQueryPlanner().getSchema().getFunctions(path.getDisplay(), false);
  ...
  private boolean materializeSelfQuery(SqrlSqlQuery node) {
    ...
    // don't materialize self if we have external arguments, the query will be inlined or called
    // from gql
    return !(node.getTableArgs().isPresent()
            && node.getTableArgs().get().getParameters().size() > 0)
        ||
  ...
  public Void visit(SqrlExpressionQuery node, Void context) {
    ...
    if (node.getIdentifier().names.size() > 2) {
      Name last =
          nameUtil.toName(node.getIdentifier().names.get(node.getIdentifier().names.size() - 2));
      ...
      throw addError(ErrorLabel.GENERIC, node.getExpression(), "Cannot assign expression to root");
    }
    ...
    selectList.add(
        SqlStdOperatorTable.AS.createCall(
            ...
            node.getExpression(),
            new SqlIdentifier(
                node.getIdentifier().names.get(node.getIdentifier().names.size() - 1),
                SqlParserPos.ZERO)));
    ...
    SqlSelect select =
        new SqlSelectBuilder()
            ...
            .build();
    validateTable(node, select, node.getTableArgs(), true);
    ...
    RexNode rexNode =
        framework.getQueryPlanner().planExpression(node.getExpression(), table.get().getRowType());
  ...
  public Void postvisit(SqrlAssignment assignment, Void context) {
    ...
    NormalizeTablePath normalizeTablePath =
        new NormalizeTablePath(
            planner.getCatalogReader(),
            getParamMapping(),
            new SqlNameUtil(planner.getFramework().getNameCanonicalizer()),
            errorCollector);
    SqrlToSql sqrlToSql =
        new SqrlToSql(
            planner,
            ...
            normalizeTablePath,
            getParameters().get(assignment),
            framework.getSchema().getUniquePkId(),
            nameUtil);
    ...
    if (assignment instanceof SqrlJoinQuery) {
      List<SqrlTableMacro> isASqrl =
          isA.stream().map(f -> ((SqrlTableMacro) f)).collect(Collectors.toList());
      NamePath path = nameUtil.toNamePath(assignment.getIdentifier().names);
      ...
      Relationship rel =
          new Relationship(
              ...
              path,
              toTable,
              Relationship.JoinType.JOIN,
              Multiplicity.MANY,
              result.getParams(),
              nodeSupplier,
              hasTestHint(assignment.getHints()));
      ...
      Optional<Supplier<RelNode>> nodeSupplier =
          result.getParams().isEmpty() ? Optional.empty() : Optional.of(() -> rel);
   ...
   *
   * <p>Find all SELECT * on direct tables, these are possible Aliases.
   *
   * <p>Assure columns are named and no collisions.
   *
   * <p>We need a global PK incrementer to allow for multiple select * and have it still work (UUID
   * collisions)
   *
   * <p>If it errors completely: Derive type field names, give ANY
   *
   * <p>Allow all function resolution during validation, no need to plan, give any type
   *
   * <p>Allow UNION access tables
   */
  public void validateTable(
      SqrlAssignment statement,
      SqlNode query,
    ...
    Pair<List<FunctionParameter>, SqlNode> argResult =
        transformArgs(
            parentTable,
            query,
            materializeSelf,
    ...
    if (statement.getIdentifier().names.size() > 1) {
      ...
      if (sqrlTable.isEmpty()) {
        throw addError(
            ErrorLabel.GENERIC,
            statement.getIdentifier().getComponent(statement.getIdentifier().names.size() - 1),
    ...
    if (statement instanceof SqrlExpressionQuery) {
      ...
      if (aggregate != null) {
        throw addError(ErrorLabel.GENERIC, aggregate, "Aggregate functions not yet allowed");
      }
    }
  }

  private boolean validateTopLevelNamed(SqlNode sqlNode) {
    return SqlNodeVisitor.accept(
        new SqlTopLevelRelationVisitor<Boolean, Object>() {
  ...
  private void validateHasNestedSelf(SqlNode query) {
    if (query.getKind() == SqlKind.UNION) {
      ...
      throw addError(
          ErrorLabel.GENERIC, query, "Unknown nested query type. Must be SELECT or JOIN");
    }
  ...
  public static boolean isSelfTable(SqlNode sqlNode) {
    if (sqlNode instanceof SqlCall
      ...
      SqlIdentifier id = ((SqlIdentifier) ((SqlCall) sqlNode).getOperandList().get(0));
      return id.names.size() == 1 && id.names.get(0).equalsIgnoreCase("@");
    }
    return false;
  }

  private Pair<List<FunctionParameter>, SqlNode> transformArgs(
      Optional<SqrlTableMacro> parentTable,
      SqlNode query,
      boolean materializeSelf,
      SqrlTableFunctionDef sqrlTableFunctionDef) {
    List<FunctionParameter> parameterList =
        toParams(
            sqrlTableFunctionDef.getParameters(), framework.getQueryPlanner().createSqlValidator());

    SqlNode node =
        SqlNodeVisitor.accept(
            new SqlRelationVisitor<>() {
              ...
              public SqlNode visitQuerySpecification(SqlSelect node, Object context) {
                return new SqlSelectBuilder(node)
                    ...
                    .rewriteExpressions(
                        rewriteVariables(parentTable, parameterList, materializeSelf))
                    .build();
              }
              ...
              public SqlNode visitAliasedRelation(SqlCall node, Object context) {
                ...
                return node.getOperator()
                    .createCall(node.getParserPosition(), ops.toArray(SqlNode[]::new));
              }
              ...
              private SqlNode visitAugmentedTable(SqlCall node, Object context) {
                ...
                return node.getOperator()
                    .createCall(node.getFunctionQuantifier(), node.getParserPosition(), op);
              }
              ...
              public SqlNode visitCall(SqlCall node, Object context) {
                throw addError(
                    ErrorLabel.GENERIC, node, "Unsupported call: %s", node.getOperator().getName());
              }
            },
            query,
            null);
  ...
  public SqlShuttle rewriteVariables(
      Optional<SqrlTableMacro> parentTable,
      List<FunctionParameter> parameterList,
      boolean materializeSelf) {
        ...
        if (isSelfField(nameUtil.toNamePath(id.names)) && !materializeSelf) {
          ...
          RelDataTypeField field =
              framework
                  ...
                  .getCatalogReader()
                  .nameMatcher()
                  .field(parentTable.get().getRowType(), name);
          if (field == null) {
            throw addError(ErrorLabel.GENERIC, id, "Cannot find field on parent table");
          }
          SqrlFunctionParameter functionParameter =
              new SqrlFunctionParameter(
                  ...
                  field.getType(),
                  true,
                  new UnknownCaseParameter(name));
          parameterList.add(functionParameter);
          SqlDynamicParam param =
              new SqlDynamicParam(functionParameter.getOrdinal(), id.getParserPosition());
   ...
   *
   * <p>Append an alias.* to the end so we can use the same logic to determine what it points to.
   */
  @Override
  public Void visit(SqrlJoinQuery node, Void context) {
    ...
    if (parent.isEmpty()) {
      throw addError(
          ErrorLabel.GENERIC, node.getIdentifier(), "Cannot assign join declaration on root");
    }
    ...
    if (lastAlias.isEmpty()) {
      throw addError(
          ErrorLabel.GENERIC,
          node.getQuery(),
          "Not a valid join declaration. "
              + "Could not derive table/alias of last join path item.");
    }

    SqlSelect select =
        new SqlSelectBuilder(node.getQuery())
            .setSelectList(
                List.of(new SqlIdentifier(List.of(lastAlias.get(), ""), SqlParserPos.ZERO)))
            .build();
   ...
   *
   * <p>Must have Order by statement
   */
  @Override
        ...
        (t) -> {
          ...
          if (modTable == null) {
            addError(
                ErrorLabel.GENERIC,
                node,
                "Table cannot have a column added: %s",
                names.getDisplay());
          } else if (modTable.isLocked()) {
            addError(
                ErrorCode.TABLE_LOCKED,
                node,
                "Cannot add column to locked table: %s",
                names.getDisplay());
          }
        });
  ...
  private static List<FunctionParameter> toParams(
      List<SqrlTableParamDef> params, SqlValidator validator) {
            ...
            .map(
                p ->
                    new SqrlFunctionParameter(
                        ...
                        p.getType(),
                        p.getIndex(),
                        p.getType().deriveType(validator),
                        false,
                        new NoParameter()))
            .collect(Collectors.toList());
    return parameters;
  ...
  public static RuntimeException addError(
    ...
    RuntimeException exception =
        CheckUtil.createAstException(
            errorCode, node, format == null ? message : String.format(message, format));
    return errorCollector.handle(exception);
  }

  public RuntimeException addError(
      Throwable cause, ErrorLabel errorCode, SqlNode node, String message, Object... format) {
    RuntimeException exception =
        CheckUtil.createAstException(
            Optional.of(cause),
            errorCode,
  ...
  private RuntimeException addError(
      ErrorLabel errorCode, SqlNode node, String message, Object... format) {
    return addError(errorCollector, errorCode, node, message, format);
  }

  private void addWarn(ErrorLabel errorCode, SqlNode node, String message, Object... format) {
    // todo: warn
  ...
  public void plan(MainScript mainScript, ModuleLoader composite) {
    ...
    try {
      scriptNode =
          (ScriptNode) framework.getQueryPlanner().parse(Dialect.SQRL, mainScript.getContent());
    } catch (Exception e) {
    ...
    for (SqlNode statement : scriptNode.getStatements()) {
      try {
        ErrorCollector lineErrors =
            scriptErrors.atFile(SqrlAstException.toLocation(statement.getParserPosition()));
        errorCollector = lineErrors;
        ...
        ErrorCollector statementErrors =
            scriptErrors.atFile(SqrlAstException.toLocation(statement.getParserPosition()));
  ...
  private boolean hasTestHint(Optional<SqlNodeList> optionalStmt) {
    return optionalStmt.isPresent()
        && optionalStmt.get().getList().stream()
            .anyMatch(
                node ->
                    node instanceof SqlHint
                        && TEST_HINT_NAME.equalsIgnoreCase(((SqlHint) node).getName()));
  }
}
