...
public abstract class SqlNodeVisitor<R, C>
    implements SqlRelationVisitor<R, C>, StatementVisitor<R, C> {
  ...
  public static <R, C> R accept(SqlTopLevelRelationVisitor<R, C> visitor, SqlNode node, C context) {
    ...
    } else if (node instanceof SqlCall && SqlKind.SET_QUERY.contains(node.getKind())) {
      return visitor.visitSetOperation((SqlCall) node, context);
    } else if (node instanceof SqlCall
        && node.getKind() == SqlKind.ORDER_BY
  ...
  public static <R, C> R accept(SqlRelationVisitor<R, C> visitor, SqlNode node, C context) {
    ...
    if (node.getKind() == SqlKind.AS) {
      ...
      return visitor.visitSetOperation((SqlCall) node, context);
        ...
        && ((SqlCall) node).getOperator() instanceof SqlCollectionTableOperator) {
      return visitor.visitCollectTableFunction((SqlCall) node, context);
    } else if (node instanceof SqlCall
        && ((SqlCall) node).getOperator() instanceof SqlLateralOperator) {
      return visitor.visitLateralFunction((SqlCall) node, context);
    } else if (node instanceof SqlCall
        && ((SqlCall) node).getOperator() instanceof SqlUnnestOperator) {
