...
public class SqrlToSql implements SqlRelationVisitor<Result, Context> {
  ...
  @Getter private final Multimap<SqlNode, TableFunction> isA = ArrayListMultimap.create();
  ...
  public Result rewrite(SqlNode query, boolean materializeSelf, NamePath currentPath) {
    Context context =
        new Context.ContextBuilder(materializeSelf, currentPath, currentPath.size() > 0).build();
  ...
  public Result visitQuerySpecification(SqlSelect call, Context context) {
    ...
    Context newContext =
        new Context.ContextBuilder(context)
    ...
    if (isDistinctOnHintPresent(call)) {
      List<Integer> hintOps =
          IntStream.range(0, call.getSelectList().size()).boxed().collect(Collectors.toList());
      SqlSelectBuilder sqlSelectBuilder =
          new SqlSelectBuilder(call).setLimit(1).clearKeywords().setFrom(result.sqlNode);
      ...
      SqlSelect top =
          new SqlSelectBuilder()
              ...
              .build();
      return new Result(
          top, result.getCurrentPath(), List.of(), List.of(), Optional.empty(), result.params);
      ...
      Set<Integer> pkColumns =
          context.isNested() ? getPrimaryKeyColumns(tableFromPath.get()) : Set.of();

      SqlSelectBuilder inner =
          new SqlSelectBuilder(call)
              .clearKeywords()
      ...
      result.condition.ifPresent(inner::appendWhere);
      SqlSelectBuilder topSelect =
          new SqlSelectBuilder()
              .setFrom(inner.build())
              .setTopNHint(
                  call.isKeywordPresent(SqlSelectKeyword.DISTINCT)
                      ? Type.SELECT_DISTINCT
                      : Type.TOP_N,
                  SqlSelectBuilder.sqlIntRange(pkColumns));
    ...
    result.condition.ifPresent(select::appendWhere);
  ...
  private boolean containsFieldName(
      Collection<String> fieldNames, String column, SqlNameMatcher nameMatcher) {
      ...
      if (node instanceof SqlIdentifier) {
        ...
        if (ident.isStar() && ident.names.size() == 1) {
          for (NamePath path : newContext.getAliasPathMap().values()) {
            Collection<Function> sqrlTable =
                planner.getSchema().getFunctions(path.getDisplay(), false);

            if (!sqrlTable.isEmpty()) {
              new ArrayList<>(sqrlTable)
                  .stream().map(f -> (SqrlTableMacro) f).forEach(f -> isA.put(call, f));
            }
          }
        } else if (ident.isStar() && ident.names.size() == 2) {
          NamePath path =
              newContext
                  .getAliasPath(nameUtil.toName(ident.names.get(0)))
                  .orElseThrow(
                      () -> new RuntimeException("Could not find alias: " + ident.names.get(0)));
          Collection<Function> sqrlTable =
              planner.getSchema().getFunctions(path.getDisplay(), false);
  ...
  private void pullUpKeys(
      SqlSelectBuilder inner, List<PullupColumn> keysToPullUp, boolean isAggregating) {
  ...
  public Result visitAliasedRelation(SqlCall node, Context context) {
    ...
    SqlNode newNode = aliasBuilder.setTable(result.getSqlNode()).build();

    return new Result(
        newNode,
        result.getCurrentPath(),
        result.pullupColumns,
        List.of(),
        result.getCondition(),
        result.params);
  }
  ...
  public Result visitTable(SqlIdentifier node, Context context) {
    List<SqlNode> items =
        node.names.stream()
            .map(
                name ->
                    new SqlIdentifier(
                        name, node.getComponentParserPosition(node.names.indexOf(name))))
    ...
    List<PullupColumn> pullupColumns =
        (context.isNested && result.getPathItems().get(0) instanceof SelfTablePathItem)
    ...
    if (!pullupColumns.isEmpty() || result.getPathItems().size() > 1) {
      sqlNode =
          buildAndProjectLast(
              pullupColumns,
              sqlNode,
              result.getPathItems().get(0),
              result.getPathItems().get(result.getPathItems().size() - 1));
    }

    this.parameters = result.getParams(); // update parameters with new parameters
    return new Result(
        sqlNode, result.getPath(), pullupColumns, List.of(), Optional.empty(), result.getParams());
  }

  public SqlNode buildAndProjectLast(
      List<PullupColumn> pullupCols, SqlNode sqlNode, PathItem first, PathItem last) {

    SqlSelectBuilder select = new SqlSelectBuilder().setFrom(sqlNode);
    ...
    for (PullupColumn column : pullupCols) {
      selectList.add(
          SqlStdOperatorTable.AS.createCall(
              SqlParserPos.ZERO,
              new SqlIdentifier(
                  List.of(first.getAlias().getDisplay(), column.getColumnName()),
                  SqlParserPos.ZERO),
        ...
        .map(
            i ->
                new PullupColumn(
                    ...
                    String.format(
                        "%spk%d_%s",
                        ReservedName.SYSTEM_HIDDEN_PREFIX,
                        uniquePkId.incrementAndGet(),
                        table.getRowType().getFieldList().get(i).getName()),
                    String.format(
                        "%spk%d_%s",
                        ReservedName.SYSTEM_HIDDEN_PREFIX,
  ...
  public Result visitJoin(SqlJoin call, Context context) {
    // Check if we should skip the lhs, if it's self and we don't materialize and there is no
    // condition
    if (isSelfTable(call.getLeft()) && !context.isMaterializeSelf()) {
    ...
    Context rhsContext =
        new Context.ContextBuilder(context)
            ...
            .build();
    Result rightNode =
        SqlNodeVisitor.accept(this, appendAliasIfRequired(call.getRight()), rhsContext);

    SqlNode join =
        new SqlJoinBuilder(call)
    ...
    return new Result(
        join,
        rightNode.getCurrentPath(),
        leftNode.pullupColumns,
        List.of(),
  ...
  public Result visitSetOperation(SqlCall node, Context context) {
    final List<Result> operandResults =
        node.getOperandList().stream()
            .map(o -> SqlNodeVisitor.accept(this, o, context))
            .collect(Collectors.toList());

    Optional<SqlNode> condition =
        operandResults.stream().map(r -> r.condition).flatMap(Optional::stream).findAny();
    if (condition.isPresent()) {
      throw new RuntimeException("Trailing condition abandoned in rewriting");
    }

    Optional<PullupColumn> pullupColumn =
        operandResults.stream().map(Result::getPullupColumns).flatMap(Collection::stream).findAny();
    ...
    List<NamePath> tableReferences =
        operandResults.stream()
            .map(o -> o.tableReferences)
            .flatMap(Collection::stream)
            .collect(Collectors.toList());

    List<SqlNode> operands =
        operandResults.stream().map(Result::getSqlNode).collect(Collectors.toList());

    SqlCall call = new SqlCallBuilder(node).setOperands(operands).build();
  ...
  public Result visitAugmentedTable(SqlCall node, Context context) {
    ...
    SqlCall call =
        node.getOperator()
            .createCall(node.getFunctionQuantifier(), node.getParserPosition(), result.sqlNode);
    return new Result(
        call,
        result.currentPath,
        result.pullupColumns,
  ...
  public Result visitValues(SqlCall node, Context context) {
    return new Result(node, context.currentPath, List.of(), List.of(), Optional.empty(), List.of());
  }
  ...
  public class WalkExpressions extends SqlShuttle {
    ...
    public SqlNode visit(SqlIdentifier id) {
      Preconditions.checkState(
          !isVariable(nameUtil.toNamePath(id.names)), "Found variable when expecting one.");
  ...
  public static class Context {
    ...
    public static class ContextBuilder {
      ...
      public ContextBuilder(boolean materializeSelf, NamePath currentPath, boolean isNested) {
      ...
      public ContextBuilder setAliasPathMap(Map<Name, NamePath> aliasPathMap) {
