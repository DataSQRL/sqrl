...
public class NormalizeTablePath {
  ...
  public NormalizeTablePath(
      CatalogReader catalogReader,
      Map<FunctionParameter, SqlDynamicParam> paramMapping,
  ...
  public TablePathResult convert(
      List<SqlNode> items, Context context, List<FunctionParameter> parameters) {
  ...
  private List<PathItem> mapToPathItems(
      Context context, List<SqlNode> items, List<FunctionParameter> params, PathWalker pathWalker) {
    ...
    if (getTable(identifier).isPresent()) {
      ...
      List<SqlNode> args =
          rewriteInternalArgs(identifier, internalParams, context, params, parent.getRowType());

      pathItems.add(
          new TableFunctionPathItem(
      ...
      throw addError(errors, ErrorLabel.GENERIC, item, "Could not find table: %s", identifier);
    }
    ...
    while (input.hasNext()) {
      ...
      if (!hasTableFnc) {
        throw addError(
            errors, ErrorLabel.GENERIC, item, "Could not find table: %s", pathWalker.getPath());
      }
      List<SqrlFunctionParameter> internalParams = getInternalParamsOfTable(pathWalker.getPath());

      List<SqlNode> args =
          rewriteInternalArgs(alias, internalParams, context, params, parentTable.getRowType());
  ...
  private List<SqrlFunctionParameter> getInternalParamsOfTable(NamePath path) {
    return getTable(path).get().getParameters().stream()
  ...
  private Optional<SqrlTableMacro> getTable(String identifier) {
    ArrayList<Function> functions =
        new ArrayList<>(catalogResolver.getSchema().getFunctions(identifier, false));
  ...
  private List<SqlNode> rewriteInternalArgs(
      Name alias,
      List<SqrlFunctionParameter> internalParams,
      Context context,
      List<FunctionParameter> params,
      RelDataType parentRowType) {
    List<SqlNode> nodes = new ArrayList<>();
    for (SqrlFunctionParameter param : internalParams) {
      Optional<String> parentParameterName =
          param.getParentName().resolve(parentRowType, catalogResolver.nameMatcher());
      if (parentParameterName.isEmpty()) {
        throw new RuntimeException(
            "Internal error, cannot find parent parameter name: " + param.getVariableName());
      }
      SqlIdentifier identifier =
          new SqlIdentifier(
              List.of(alias.getDisplay(), parentParameterName.get()), SqlParserPos.ZERO);
      SqlNode rewritten =
          context.isMaterializeSelf() ? identifier : rewriteToDynamicParam(identifier, params);
      nodes.add(rewritten);
    }
    return nodes;
  }
  ...
  public SqlNode rewriteToDynamicParam(SqlIdentifier id, List<FunctionParameter> params) {
    ...
    SqrlFunctionParameter functionParameter =
        new SqrlFunctionParameter(
            id.names.get(1),
            Optional.empty(),
            SqlDataTypeSpecBuilder.create(anyType),
            params.size(),
            anyType,
            true,
  ...
  private Optional<Name> getIdentifier(SqlNode item) {
    if (item instanceof SqlIdentifier) {
      return Optional.of(((SqlIdentifier) item).getSimple()).map(nameUtil::toName);
    }
