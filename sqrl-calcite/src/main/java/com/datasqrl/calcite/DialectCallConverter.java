...
public class DialectCallConverter {
  ...
  public static final Map<Name, RuleTransform> transformMap =
      ServiceLoaderDiscovery.getAll(RuleTransform.class).stream()
  ...
  public RelNode convert(Dialect dialect, RelNode relNode) {
    ...
    relNode =
        Programs.hep(rules, false, null)
            .run(planner, relNode, relNode.getTraitSet(), List.of(), List.of());
          ...
          public RelNode visit(LogicalAggregate aggregate) {
            for (AggregateCall call : aggregate.getAggCallList()) {
              RuleTransform transform =
                  transformMap.get(Name.system(call.getAggregation().getName()));
              if (transform != null) {
            ...
            parent.accept(
                new RexShuttle() {
                  ...
                  public RexNode visitCall(RexCall call) {
                    RuleTransform transform =
                        transformMap.get(Name.system(call.getOperator().getName()));
                    if (transform != null) {
