...
public class PathToSql {
  ...
  public SqlNode build(List<PathItem> pathItems) {
    for (var i = 0; i < pathItems.size(); i++) {
      var item = pathItems.get(i);
      if (item instanceof SelfTablePathItem) {
        var tableName = ((SelfTablePathItem)item).getTable().getQualifiedName().get(0);
        SqlNode table = new SqlIdentifier(tableName, SqlParserPos.ZERO);
        var aliasedCall = SqlStdOperatorTable.AS.createCall(
            SqlParserPos.ZERO,
            table,
            new SqlIdentifier(ReservedName.SELF_IDENTIFIER.getCanonical(), SqlParserPos.ZERO)
        );
        stack.push(aliasedCall);
      } else if (item instanceof TableFunctionPathItem tableFncItm) {
        ...
        var call = fun.createCall(SqlParserPos.ZERO, tableFncItm.getArguments());
