...
public class SwaggerContainerIT extends SqrlContainerTestBase {
  ...
  private void testSwaggerJsonEndpoint() {
    ...
    assertThat(jsonResponse.has("openapi"))
        .as("Response should contain OpenAPI version")
        .isTrue();
    assertThat(jsonResponse.has("info"))
        .as("Response should contain API info")
        .isTrue();
    assertThat(jsonResponse.has("paths"))
        .as("Response should contain API paths")
        .isTrue();
    ...
    assertThat(paths.has("/rest/sensors/readings"))
        .as("SecReading REST endpoint should be documented")
        .isTrue();
    assertThat(paths.has("/rest/sensors/maxtemp"))
        .as("SensorMaxTemp REST endpoint should be documented")
        .isTrue();

    // Verify endpoint details
    var readingsPath = paths.get("/rest/sensors/readings");
    assertThat(readingsPath.has("get"))
        .as("Readings endpoint should have GET method")
        .isTrue();

    var maxTempPath = paths.get("/rest/sensors/maxtemp");
    assertThat(maxTempPath.has("get"))
        .as("MaxTemp endpoint should have GET method")
        .isTrue();
  }
  ...
  private void testRestAPIEndpoints() {
    ...
    assertThat(readingsJson.has("data"))
        .as("REST response should contain data field")
        .isTrue();
    ...
    assertThat(maxTempJson.has("data"))
        .as("REST response should contain data field")
        .isTrue();
