...
/**
 * A {@link SourceDataset} defines a group of {@link SourceTable} that comprise one semantically
 * coherent dataset.
 * <p>
 * The role of a {@link SourceDataset} is to register all of its {@link SourceTable} within an
...
public class SourceDataset {
  ...
  public synchronized SourceTable addTable(SourceTableConfiguration tableConfig,
    ...
    if (table == null) {
      ...
      errors.fatal("Table [%s] already exists. To update table, delete and re-add",
          tblName.getDisplay());
      return null;
    }
