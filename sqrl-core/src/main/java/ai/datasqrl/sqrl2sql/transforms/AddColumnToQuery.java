package ai.datasqrl.sqrl2sql.transforms;
...
import ai.datasqrl.sqrl2sql.AliasGenerator;
...
public class AddColumnToQuery {
  ...
  /**
   * Adds an expression and joins back to the main query, taking careful account of shadowing.
   * <p>
   ...
   * <p>
   * Transform: SELECT _t1._uuid, _t1._idx1, _t1.quantity, _t1.unit_price, _t1.discount,
   * _t2.discount AS discount$1 FROM entries AS _t1 INNER JOIN (SELECT _uuid, _idx1,
   * COALESCE(discount, 0.0) AS discount FROM entries) AS _t2 ON _t1._uuid = _t2._uuid AND _t1._idx1
   * = _t2._idx1;
   */
  public QuerySpecification transform(Table table, Name expressionName,
    ...
    TableNode tableNode = new TableNode(Optional.empty(), table.getId().toNamePath(),
        Optional.of(lAlias));
