...
public class TypeHelper {
  ...
  public static <F extends TypedField> F getNestedField(@NonNull RelationType<F> relation,
    ...
    for (int i = 0; i < path.getLength(); i++) {
      if (i > 0) {
        base = asRelation(field.getType());
      }
      field = base.getFieldByName(path.get(i));
      if (field == null) {
        throw new IllegalArgumentException(
            String.format("Not a valid path [%s] within type hierarchy [%s]", path, relation));
      }
    }
    return field;
  }
  ...
  public static RelationType getNestedRelation(RelationType<? extends TypedField> relation,
      NamePath path) {
  ...
  public static Type unfurlType(Type type) {
    if (type instanceof ArrayType) {
      return unfurlType(((ArrayType) type).getSubType());
    }
    return type;
  }
