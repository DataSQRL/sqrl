...
public class UniversalTableBuilder {
    ...
    public UniversalTableBuilder(@NonNull Name name, @NonNull NamePath path, UniversalTableBuilder parent, boolean isSingleton) {
        this.parent = parent;
    ...
    public void addChild(Name name, UniversalTableBuilder child, Relationship.Multiplicity multiplicity) {
    ...
    public<T> List<Pair<String,T>> convert(TypeConverter<T> converter, boolean includeNested, boolean onlyVisible) {
        return fields.getFields(false)
                .filter(f -> (includeNested || (f instanceof Column)) && (!onlyVisible || f.isVisible())).map(f -> {
            String name = f.getId().getCanonical();
            T type;
            if (f instanceof Column) {
                Column column = (Column) f;
                type = converter.nullable(convertType(column.getType(), converter), column.isNullable());
            } else {
                ChildRelationship childRel = (ChildRelationship) f;
                T nestedTable = converter.nestedTable(childRel.getChildTable().convert(converter, includeNested, onlyVisible));
                nestedTable = converter.nullable(nestedTable,childRel.multiplicity== Relationship.Multiplicity.ZERO_ONE);
    ...
    private<T> T convertType(RelDataType type, TypeConverter<T> converter) {
        ...
        if (subType.isPresent()) {
            ...
            return converter.wrapArray(converter.nullable(convertType(subType.get(), converter),subType.get().isNullable()));
        } else {
    ...
    public interface Factory {
        ...
        UniversalTableBuilder createTable(@NonNull Name name, @NonNull NamePath path, @NonNull UniversalTableBuilder parent, boolean isSingleton);
    ...
    public static abstract class AbstractFactory implements Factory {
        ...
        public UniversalTableBuilder createTable(@NonNull Name name, @NonNull NamePath path, @NonNull UniversalTableBuilder parent, boolean isSingleton) {
    ...
    public static class ImportFactory extends AbstractFactory {
        ...
        public UniversalTableBuilder createTable(@NonNull Name name, @NonNull NamePath path, @NonNull UniversalTableBuilder parent, boolean isSingleton) {
            ...
            if (!isSingleton && addArrayIndex) {
                tblBuilder.addColumn(ReservedName.ARRAY_IDX, withNullable(typeFactory.createSqlType(SqlTypeName.INTEGER), false));
            }
            return tblBuilder;
        }
        ...
        public UniversalTableBuilder createTable(@NonNull Name name, @NonNull NamePath path, boolean hasSourceTimestamp) {
            UniversalTableBuilder tblBuilder = new UniversalTableBuilder(name,path,1);
            tblBuilder.addColumn(ReservedName.UUID, withNullable(typeFactory.createSqlType(SqlTypeName.CHAR, 36),false));
            tblBuilder.addColumn(ReservedName.INGEST_TIME, withNullable(typeFactory.createSqlType(SqlTypeName.TIMESTAMP_WITH_LOCAL_TIME_ZONE, 3),false));
            if (hasSourceTimestamp) {
                tblBuilder.addColumn(ReservedName.SOURCE_TIME, withNullable(typeFactory.createSqlType(SqlTypeName.TIMESTAMP_WITH_LOCAL_TIME_ZONE, 3), false));
            }
            return tblBuilder;
        }
    }
