...
public class JoinBuilderImpl implements JoinBuilder {
    ...
    public JoinBuilderImpl(UniqueAliasGenerator aliasGenerator,
        JoinDeclarationContainer joinDeclarations,
        TableMapper tableMapper) {
     ...
     */
    public static SqlJoinDeclaration expandPath(TablePath path, boolean forceIncludeBase, JoinBuilderFactory joinBuilderFactory) {
    ...
    public void addFirstRel(Relationship rel /*entries*/, String baseAlias /*o*/, Optional<String> lastAlias) {
      ...
      //  sqlnode: entries e
      //  pullup: _.id = e.id
      SqlJoinDeclaration join = joinDeclarations.getDeclaration(rel);
      SqlJoinDeclaration rewritten = join.rewriteSqlNode(baseAlias /*new self alias*/, lastAlias /*if this is the last alias*/, aliasGenerator);
    ...
    public void append(Relationship rel, Optional<String> lastAlias) {
      ...
      SqlJoinDeclaration join = joinDeclarations.getDeclaration(rel);
      SqlJoinDeclaration rewritten = join.rewriteSqlNode(currentAlias, lastAlias, aliasGenerator);
