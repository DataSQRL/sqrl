...
public enum MaterializationPreference {
    ...
    public MaterializationPreference combine(MaterializationPreference other) {
        Preconditions.checkArgument(isCompatible(other),"Materialization preferences are not compatible");
        if (this==MUST || other==MUST) return MUST;
        if (this==CANNOT || other==CANNOT) return CANNOT;
        if (this==SHOULD || other==SHOULD) return SHOULD;
        return SHOULD_NOT;
    }
