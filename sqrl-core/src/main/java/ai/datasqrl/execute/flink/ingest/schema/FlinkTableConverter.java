...
public class FlinkTableConverter {
  ...
  public Pair<Schema, TypeInformation> tableSchemaConversion(
      FlexibleDatasetSchema.TableField table) {
    NamePath path = NamePath.of(table.getName());
    Schema.Builder schemaBuilder = Schema.newBuilder();
    List<String> rowNames = new ArrayList<>();
    List<TypeInformation> rowCols = new ArrayList<>();
    getFields(table.getFields()).map(
            t -> fieldTypeSchemaConversion(t.getLeft(), t.getMiddle(), t.getRight(), path))
        .forEach(p -> {
    ...
    schemaBuilder.column(ReservedName.UUID.getCanonical(),
        toFlinkDataType(UuidType.INSTANCE).notNull());
    rowNames.add(ReservedName.UUID.getCanonical());
    rowCols.add(FlinkUtilities.getFlinkTypeInfo(UuidType.INSTANCE, false));
    schemaBuilder.column(ReservedName.INGEST_TIME.getCanonical(),
        DataTypes.TIMESTAMP_LTZ(3).notNull());
    rowNames.add(ReservedName.INGEST_TIME.getCanonical());
    rowCols.add(FlinkUtilities.getFlinkTypeInfo(
        DateTimeType.INSTANCE, false));
    schemaBuilder.column(ReservedName.SOURCE_TIME.getCanonical(),
        DataTypes.TIMESTAMP_LTZ(3).nullable());
    rowNames.add(ReservedName.SOURCE_TIME.getCanonical());
    rowCols.add(FlinkUtilities.getFlinkTypeInfo(DateTimeType.INSTANCE, false));
...
//        schemaBuilder.columnByExpression("__rowtime", "CAST(_ingest_time AS TIMESTAMP_LTZ(3))");
    ...
    return Pair.of(schemaBuilder.build(),
        Types.ROW_NAMED(rowNames.toArray(new String[0]), rowCols.toArray(new TypeInformation[0])));
  }
  ...
  private Pair<DataTypes.Field, TypeInformation> fieldTypeSchemaConversion(Name name,
