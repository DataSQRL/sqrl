...
public class SqrlRounding {
    ...
    public Instant roundToMinute(Instant instant) {
        return ZonedDateTime.ofInstant(instant, ZoneId.of("UTC")).truncatedTo(ChronoUnit.MINUTES).toInstant();
    }
    ...
    public Instant roundToMonth(Instant instant) {
        return ZonedDateTime.ofInstant(instant, ZoneId.of("UTC")).with(TemporalAdjusters.firstDayOfMonth()).truncatedTo(ChronoUnit.DAYS).toInstant();
    }
