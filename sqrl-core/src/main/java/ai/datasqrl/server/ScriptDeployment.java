...
public class ScriptDeployment implements Serializable {
  ...
  ScriptDeployment(@NonNull UUID uuid, @NonNull Instant submissionTime,
      @NonNull ScriptBundle scriptBundle) {
    this.uuid = uuid;
  ...
  public Result getStatusResult(StreamEngine streamEngine,
      Optional<CompilationResult> compileResult) {
    ...
    if (compileResult.isPresent()) {
      builder.compilation(compileResult.get());
    }
