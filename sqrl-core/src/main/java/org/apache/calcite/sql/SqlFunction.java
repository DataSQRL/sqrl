...
public class SqlFunction extends SqlOperator {
  ...
  private RelDataType deriveType(
    ...
    //SQRL: convert agg function to inline-agg.
    // check if path we have a path and is an inline-agg
    // currently only allow simple inline-aggs (1 arg and it is a sqlidentifier
    if (function != null && function.isAggregator() && function instanceof ConvertableFunction
        && args.size() == 1 && args.get(0) instanceof SqlIdentifier) {
      SqlQualified qualified = scope.fullyQualify((SqlIdentifier) args.get(0));
      RelDataType type = qualified.namespace.getRowType();
      List<String> suffix = qualified.suffix();
      //todo is relationship?
      if (type.getField(suffix.get(0), false, false).getType() instanceof RelRecordType) {
        ConvertableFunction aggFunction = (ConvertableFunction) function;
        Optional<SqlFunction> fnc = aggFunction.convertToInlineAgg();
        if (fnc.isPresent()) {
          function = fnc.get();
        }
      }
