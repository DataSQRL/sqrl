...
That is why DataSQRL exists. Coding agents are impressive solution generators, but they require a **conceptual model**, **validator**, and **simulator** to ensure correctness, safety, and robustness. In most successful agentic system deployed in the wild, the neural network–driven reasoning is balanced by an external framework that supplies structure, constraints, and feedback loops.

For example, self-driving cars. Neural networks power environmental perception: recognizing lanes, traffic lights, pedestrians. Autonomous driving becomes possible only when this probabilistic perception is grounded in detailed maps, constraint systems, planning modules, and a world model based on physics and real‑world dynamics.
...
For the purposes of automating data platforms, a world model is a conceptual model of data models, data processing, and serving data to consumers. Specifically, we are building a world model for *non-transactional* data processing and serving. 

The world model provides the frame of reference for implementing safe, reliable data processing systems.
It's [Database Systems: The Complete Book](http://infolab.stanford.edu/~ullman/dscb.html) combined with 25 years of data engineering experience.
...
The relational model is widely adopted, proven, and provides a solid mathematical foundation. Most LLM are trained on lots of SQL code and related documentation. And it is easy for humans to read. Modern versions of SQL (e.g. the SQL:2023 standard) support semi-structured data (JSON), polymorphic table functions, and complex pattern matching to address the messy reality of data platforms.
...
In addition to data processing, a critical function of data platforms is serving data to consumers as data streams, datasets, or data API. Data APIs, in particular, are becoming more important with the rise of operational analytics and MCP (model context protocol) for making data accessible to AI agents.
...
* **Server**: for capturing and exposing data through an API
  * **Cache**: sit between server and database to speed up frequent queries over less-frequently changing data.
...
<img src="/img/screenshots/banking_dag.png" alt="An example physical model DAG >|" width="100%"/>

#### Transpiler

While the physical model gives the AI control over what engine executes which computation, the actual mapping of logical to physical plan is done by a deterministic transpiler build in Apache Calcite. This avoids subtle bugs in data mapping and execution. The result of the transpilation are deployment assets which are executed by each engine. For example, the transpiler generates the database schema and queries for Postgres.
...
For the data serving interface we use GraphQL schema as the physical model which bidirectionally maps to the access functions, table schema, and relationships defined in the logical plan by naming convention. GraphQL fields are mapped to SQL or Kafka queries based their respective definitions in SQRL. This allows the AI to fine tune the API within GraphQL schema.
...
The world model gives AI coding agents a frame of reference to reason about data pipeline and data product implementations. DataSQRL provides analyses to support that reasoning and give users tools to validate the correctness and quality of the result.

### Verification & Introspection

Verification and introspection complements the world model by reinforcing the concepts, rules, and dependencies. DataSQRL provides analysis at 3 levels: the logical model, physical model, and deployment assets (the code that gets executed by the engines).
...
The validation component was designed to provide  validation errors with comprehensive context and suggested fixes. In our testing, this produces significantly better results compared to the AI coding agent having to look up and reason about encountered errors.

#### Physical

On compilation, DataSQRL produces the computational data flow DAG with represents the physical model. DataSQRL generates a visual representation as shown above for human validation as well as a concise textual representation that is consumed by coding agents as feedback on their proposed solutions and to reinforce the conceptual data flow of the world model.
...
<img src="/img/diagrams/automation_overview.png" alt="Archicture view of DataSQRL pipelines" width="100%"/>
...
Index structure selection is another optimization problem that is better handled by a dedicated optimizer. We use an adaptation's of [Ullman et al lattice framework for data cube selection](https://web.eecs.umich.edu/~jag/eecs584/papers/implementing_data_cube.pdf) since data cube selection and index selection are related problems with different optimization functions.

## Real World Feedback

A world model with complementary verification and introspection provides the grounding model for reasoning about data pipelines and getting feedback. However, that feedback is limited to the plan and does not account for the complexities of actual execution. Real world feedback is critical for iterative refinement of production-grade implementations and troubleshooting issues that arise in operation.
...
Simulation is important in agentic coding workflows because they allow the agent to execute and refine the implementation in a feedback loop that is executed locally and can simulate scenarios that only occur rarely on production.
...
Relating that data back to the physical model is not fully abstracted yet and requires individual setup for each deployment. This is work in progress.
...
DataSQRL supplies real-world feedback to the coding agent through the embedded simulator and its telemetry orchestration.
