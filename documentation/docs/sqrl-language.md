...
Pass-through definitions allow you to bypass SQRL's analysis and translation, passing SQL queries directly to the underlying database engine. This serves as a "backdoor" for SQL constructs that SQRL does not yet support natively.

Pass-through table, function, or relationship definition have a `RETURNS` clause in their signature that defines the result type of the query.
...
| Pass-through func | `Func(id BIGINT) RETURNS (col INT) := SELECT raw_sql WHERE id = :id;`  |
