...
API version names must start with `v` followed by a number, such as `v1` or `v2`. Each version must define either a GraphQL `schema` or an `openapi` compatibility specification, and can optionally define GraphQL `operations`. When a version defines only `openapi`, DataSQRL infers its GraphQL schema from the SQRL script.
