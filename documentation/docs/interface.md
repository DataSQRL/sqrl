...
To serve multiple API versions, configure each version under `script.api`. Each version must define either a GraphQL `schema` or an `openapi` compatibility specification. When a version defines only `openapi`, the compiler infers its GraphQL schema from the SQRL script. See [API Versioning](configuration.md#api-versioning).
