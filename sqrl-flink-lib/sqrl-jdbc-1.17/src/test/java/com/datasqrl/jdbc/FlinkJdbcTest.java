  ...
  public void testFlinkWithPostgres() throws Exception {
    ...
    try (PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>("postgres:14")) {
      ...
      String createSinkTable =
          "CREATE TABLE jdbc_sink (    arrayOfRows RAW('com.datasqrl.json.FlinkJsonType',"
      ...
      tableEnv.executeSql(
          "CREATE TEMPORARY FUNCTION IF NOT EXISTS `tojson` AS 'com.datasqrl.json.ToJson' LANGUAGE"
              + " JAVA");
      ...
      TableResult tableResult =
          tableEnv.executeSql(
              "INSERT INTO jdbc_sink SELECT tojson(arrayOfRows) AS arrayOfRows FROM"
                  + " datagen_source");
      tableResult.print();
