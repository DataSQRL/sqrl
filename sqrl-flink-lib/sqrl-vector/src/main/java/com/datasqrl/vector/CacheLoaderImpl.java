...
public class CacheLoaderImpl extends CacheLoader<String, CachedModel> implements Serializable {
  ...
  public CachedModel load(String s) throws IllegalArgumentException {
    ...
    //      Preconditions.checkArgument(Files.isRegularFile(modelPath) &&
    // Files.isReadable(modelPath),
    //          "Could not read ONNX model from file [%s]. Check file is readable and correctly
    // mounted.", modelPath);
    //      Preconditions.checkArgument(Files.isRegularFile(tokenizerPath) &&
    // Files.isReadable(tokenizerPath),
    //          "Could not read tokenizer configuration from file [%s]. Check file is readable and
    // correctly mounted.", modelPath);
    OnnxRunner runner;
    Tokenizer tokenizer;
    try {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Could not instantiate tokenizer from: " + tokenizerPath, e);
    }
