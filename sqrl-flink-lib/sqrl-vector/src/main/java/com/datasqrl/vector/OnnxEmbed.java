...
/**
 * Computes a vector embedding for the given string based on the provided ONNX vector embedding
 * model. The embedding model should be stored in a directory that is accessible at runtime.
 */
public class OnnxEmbed extends ScalarFunction {

  public LoadingCache<String, CachedModel> models =
      CacheBuilder.newBuilder().maximumSize(100).build(new CacheLoaderImpl());
  ...
  public TypeInference getTypeInference(DataTypeFactory typeFactory) {
    return TypeInference.newBuilder()
        ...
        .outputTypeStrategy(
            callContext -> Optional.of(DataTypes.of(FlinkVectorType.class).toDataType(typeFactory)))
        .build();
  }
