...
public class FlexibleSchemaValidator implements SchemaValidator, Serializable {
  ...
  public FlexibleSchemaValidator(
      @NonNull FlexibleTableSchema tableSchema,
  ...
  public SourceRecord.Named verifyAndAdjust(SourceRecord.Raw record, ErrorCollector errors) {
    ...
    errors.checkFatal(
        record.hasUUID(),
        "Input record does not have UUID: %s [table=%s]",
        record,
        tableSchema.getName());
    errors.checkFatal(
        record.getIngestTime() != null,
        "Input record does not ingest timestamp: %s [table=%s]",
        record,
        tableSchema.getName());
    errors.checkFatal(
        !hasSourceTimestamp || record.getSourceTime() != null,
        "Input record does not have source timestamp: %s [table=%s]",
        record,
        tableSchema.getName());
  ...
  private Map<Name, Object> verifyAndAdjust(
    ...
    for (Map.Entry<String, Object> entry : relationData.entrySet()) {
      ...
      if (field.isEmpty()) {
        if (!settings.dropFields()) {
          errors.fatal(
              "Field is not defined in schema: %s [table=%s]", name, tableSchema.getName());
        }
      } else {
  ...
  private Pair<Name, Object> handleNull(FlexibleFieldSchema.Field field, ErrorCollector errors) {
    ...
    for (FlexibleFieldSchema.FieldType ft : field.getTypes()) {
      if (ft.getArrayDepth() > 0 && settings.null2EmptyArray()) {
        return ImmutablePair.of(
            FlexibleSchemaHelper.getCombinedName(field, ft),
            deepenArray(Collections.EMPTY_LIST, ft.getArrayDepth() - 1));
      }
    }
    errors.fatal(
        "Field [%s] has non-null constraint but record contains null value [table=%s]",
        field.getName(), tableSchema.getName());
    return null;
  }

  private static BasicType detectType(
      Map<String, Object> originalComposite, List<FlexibleFieldSchema.FieldType> ftypes) {
    return detectTypeInternal((t, d) -> t.conversion().detectType(d), originalComposite, ftypes);
  }

  private static BasicType detectType(String original, List<FlexibleFieldSchema.FieldType> ftypes) {
    return detectTypeInternal((t, d) -> t.conversion().detectType(d), original, ftypes);
  }

  private static <O> BasicType detectTypeInternal(
      BiPredicate<BasicType, O> typeFilter, O data, List<FlexibleFieldSchema.FieldType> ftypes) {
    return ftypes.stream()
        .filter(t -> t.getType() instanceof BasicType)
        .map(t -> (BasicType) t.getType())
        .filter(t -> typeFilter.test(t, data))
        .findFirst()
        .orElse(null);
  }

  private Pair<Name, Object> verifyAndAdjust(
      Object data, FlexibleFieldSchema.Field field, ErrorCollector errors) {
    if (field.getTypes().get(0).getType().equals(ObjectType.INSTANCE)) {
      return ImmutablePair.of(
          FlexibleSchemaHelper.getCombinedName(field, field.getTypes().get(0)), data);
    }
    ...
    Optional<Simple> typeSignatureOpt =
        TypeSignatureUtil.detectSimpleTypeSignature(
            data, s -> detectType(s, types), m -> detectType(m, types));
    ...
    if (match != null) {
      ...
      errors.notice(
          "Cannot match field data [%s] onto schema field [%s], hence field is ignored [table=%s]",
          data, field.getName(), tableSchema.getName());
      return null;
    }
  }

  private Object convertDataToMatchedType(
      Object data, Type type, FlexibleFieldSchema.Field field, ErrorCollector errors) {
    if (TypeSignatureUtil.isArray(data)) {
      ...
      for (Object o : col) {
        if (o == null) {
          if (!settings.removeListNulls()) {
            errors.fatal(
                "Array contains null values: [%s] [field=%s, table=%s]",
                col, field.getName(), tableSchema);
          }
        } else {
      ...
      if (type instanceof RelationType) {
        ...
        } else if ((singletonField =
                FlexibleTypeMatcher.getSingletonBasicField((RelationType<Field>) type))
            .isPresent()) { // Singleton nested field
          Field nestedField = singletonField.get();
          FlexibleFieldSchema.FieldType nestedFieldType =
              nestedField.getTypes().get(0); // there's only one
          Map<Name, Object> result = new HashMap<>(1);
          result.put(
              FlexibleSchemaHelper.getCombinedName(nestedField, nestedFieldType),
              verifyAndAdjust(data, nestedFieldType, nestedField, 0, errors));
          return result;
        } else {
          errors.fatal(
              "Expected composite object [%s] [field=%s, table=%s]",
              data, field.getName(), tableSchema.getName());
          return null;
        }
      } else {
  ...
  private Object cast2BasicType(
      Object data, BasicType type, FlexibleFieldSchema.Field field, ErrorCollector errors) {
    if (type instanceof StringType) {
      if (data instanceof String) {
        ...
        if (!settings.castDataType()) {
          errors.fatal(
              "Expected string but got: %s [field=%s, table=%s]",
              data, field.getName(), tableSchema.getName());
        }
    ...
    try {
      ...
      errors.fatal(
          "Could not convert [%s] to type [%s] [field=%s, table=%s]",
          data, type, field.getName(), tableSchema.getName());
      return null;
    }
  }
  ...
  private Object verifyAndAdjust(
      Object data,
    ...
    if (detectedArrayDepth < type.getArrayDepth()) {
      if (settings.deepenArrays()) {
        ...
        errors.fatal(
            "Array [%s] does not have same dimension as schema [%s] [field=%s, table=%s]",
            data, type, field.getName(), tableSchema.getName());
      }
    }
    return data;
