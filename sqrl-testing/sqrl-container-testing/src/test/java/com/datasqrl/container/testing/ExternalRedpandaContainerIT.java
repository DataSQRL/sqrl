...
public class ExternalRedpandaContainerIT extends SqrlContainerTestBase {
  ...
  void givenExternalRedpandaContainer_whenCompileAndRunFlinkKafka_thenUsesExternalKafka() {
    ...
    log.info("DataSQRL container environment: KAFKA_HOST={}, KAFKA_PORT={}, PROPERTIES_BOOTSTRAP_SERVERS={}", 
        REDPANDA_CONTAINER_NAME, REDPANDA_PORT, REDPANDA_CONTAINER_NAME + ":" + REDPANDA_PORT);
    cmd.start();
  ...
  private void startRedpandaContainer() {
    ...
    redpandaContainer = new GenericContainer<>(DockerImageName.parse(REDPANDA_IMAGE))
        ...
        .withCommand(
            ...
            "--reserve-memory", "0M",
            "--node-id", "0",
            "--kafka-addr", "0.0.0.0:" + REDPANDA_PORT,
            "--advertise-kafka-addr", REDPANDA_CONTAINER_NAME + ":" + REDPANDA_PORT,
            "--pandaproxy-addr", "0.0.0.0:8082",
            "--advertise-pandaproxy-addr", REDPANDA_CONTAINER_NAME + ":8082",
            "--schema-registry-addr", "0.0.0.0:8081",
            "--rpc-addr", "0.0.0.0:33145",
            "--advertise-rpc-addr", REDPANDA_CONTAINER_NAME + ":33145",
            "--check=false"
        )
  ...
  private GenericContainer<?> createCmdContainerWithExternalKafka() {
    var container = createCmdContainer(testDir, true)
        .withNetwork(sharedNetwork)
    ...
    if (bankingDir.toFile().exists()) {
      container = container.withFileSystemBind(
          bankingDir.toString(), 
          BUILD_DIR + "/../banking", 
          BindMode.READ_ONLY
      );
