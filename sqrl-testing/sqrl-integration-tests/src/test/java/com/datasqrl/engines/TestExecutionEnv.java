...
public class TestExecutionEnv implements TestEngineVisitor<Void, TestEnvContext> {
  ...
  public Void visit(PostgresTestEngine engine, TestEnvContext context) {
    ...
    Map map =
        new ObjectMapper()
            .readValue(rootDir.resolve("build/plan/postgres.json").toFile(), Map.class);
  ...
  public Void visit(TestTestEngine engine, TestEnvContext context) {
    DatasqrlTest test = new DatasqrlTest(null, context.rootDir.resolve("build/plan"), context.env);
    try {
      int run = test.run();
      if (run != 0) {
        fail();
