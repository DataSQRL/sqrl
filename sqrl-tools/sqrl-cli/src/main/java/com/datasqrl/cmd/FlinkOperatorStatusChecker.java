...
public class FlinkOperatorStatusChecker {
    ...
    private String FLINK_REST_URL = "http://localhost:8081"; // Adjust to your Flink REST URL
    private String JOB_ID = "<your-deployment-id-here>"; // Set your Flink job ID
    private long POLLING_INTERVAL_MS = 5000; // Poll every 5 seconds
...
//        long busyTime = metrics.get("accumulated-busy-time").longValue();
        ...
        System.out.println("Vertex: " + vertex.get("name").asText() + ", Read: " + readRecords + ", Write: " + writeRecords);
    ...
    public static boolean hasMetricsStopped(String vertexId, long currentReadRecords, long currentWriteRecords) {
        ...
        System.out.printf("%s %d %d %d %d%n",vertexId, currentReadRecords, previous.readRecords, currentWriteRecords, previous.writeRecords);
    ...
    public boolean isOperatorIdle(String jobId, String vertexId) throws Exception {
        String metricsUrl = FLINK_REST_URL + "/jobs/" + jobId + "/vertices/" + vertexId + "/metrics?get=numRecordsOut";
        ...
        return hasMetricStopped(numRecordsOut); // Implement this to track if the metric has stopped increasing
    }
