...
class PublisherTest {
  ...
  @Mock private ErrorCollector errorCollector;

  @Mock private PublishRepository publishRepository;
  ...
  void testPublishSuccess_validPackageJson() {
    // Arrange
    Path validPackagePath =
        Paths.get(getClass().getClassLoader().getResource("publisher/valid").toURI());
  ...
  void testPublishFailure_invalidPackageJson() {
    // Arrange
    Path validPackagePath =
        Paths.get(getClass().getClassLoader().getResource("publisher/invalid").toURI());

    when(errorCollector.withConfig(any(Path.class))).thenReturn(errorCollector);
    when(errorCollector.abortOnFatal(any(Boolean.class))).thenReturn(errorCollector);
    when(errorCollector.exception(any(String.class), any()))
        .thenReturn(new RuntimeException("Invalid package.json"));
    ...
    verify(errorCollector, times(1))
        .fatal(any(String.class), eq("$.package: required property 'name' not found"), any());
  }
}
