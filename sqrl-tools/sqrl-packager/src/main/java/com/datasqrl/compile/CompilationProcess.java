...
public class CompilationProcess {
  private final ScriptPlanner
      planner; // 1.3 TRANSPILER: script parsing (antlr) and validation, and bookkeeping of the
               // sqrlNodes (calcite sqlNodes) of the script inside a calcite schema.
  ...
  private final ModuleLoader
      moduleLoader; // 1.2 TRANSPILER: load external modules source/sinks etc...
  private final APIConnectorManagerImpl apiConnectorManager; // manages source and sinks
  private final DAGPlanner
      dagPlanner; // 3. DAG PLANNER: planLogical (applies static cost factors based on table types
                  // and assigns tables to stages based on the cheapest cost)
  ...
  private final PhysicalPlanner
      physicalPlanner; // 5. PHYSICAL PLANNER: match preliminary physical plan (step 4) stages to
                       // configured engines to generate the physical plan
  // that bookkeeps everything needed for the engine environment (topics for kafka, tablespace for
  // postGre, flinkSQL compiled plan, ...)
  private final GraphqlPostplanHook
      graphqlPostplanHook; // 6. PHYSICAL PLANNER: walks the GraphQL schema to create the graphQL
                           // model - which encapsulates the GraphQL schema and entry points
                           // (queries, mutations, subscriptions) - using the physical plan and the
                           // database queries
  private final CreateDatabaseQueries
      createDatabaseQueries; // 1.4. TRANSPILER: Based on the logicalPlan (calcite schema) keep
                             // track of the APIQueries (wrapper that links together database query
                             // parameters, graphQL query and corresponding Calcite relNode in the
                             // logical plan) for the table functions.
  // This is useful to be able to expose data through DataBase requests and/or GraphQL requests.
  private final InferGraphqlSchema
      inferencePostcompileHook; // 1.5. TRANSPILER: generate the GraphQL schema from the logicalPlan
                                // if the user has not provided one.
  // Validate the schema and walk it to create APIQueries for managing graphQL endpoints (queries,
  // mutations, subscriptions)
  private final WriteDag
      writeDeploymentArtifactsHook; // 7. PACKAGER: generate DAG artifacts and write them to the
                                    // build dir
  ...
  private final GraphQLMutationExtraction
      graphQLMutationExtraction; // 1.1 TRANSPILER: If the user provided a GraphQL schema, extract
                                 // the mutations defined in the schema as APISource (so that all
                                 // GraphQL endpoints are APISources)
