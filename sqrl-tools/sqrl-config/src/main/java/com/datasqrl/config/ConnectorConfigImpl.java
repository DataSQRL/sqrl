...
public class ConnectorConfigImpl implements TableConfig.ConnectorConfig {
  ...
  public Optional<TableConfig.Format> getFormat() {
    ...
    //        .checkFatal(format.isPresent(), "Need to configure a format via [%s] or [%s]",
    // FORMAT_KEY,
    //            VALUE_FORMAT_KEY);
    Optional<FormatFactory> formatFactory =
        format.flatMap(
            f -> ServiceLoaderDiscovery.findFirst(FormatFactory.class, FormatFactory::getName, f));
    Optional<TableConfig.Format> format1 =
        formatFactory.map(fac -> fac.fromConfig(new EngineConfigImpl(config)));
    Optional<TableConfig.Format> defaultFormat =
        format.map(f -> new TableConfig.Format.DefaultFormat(f));
