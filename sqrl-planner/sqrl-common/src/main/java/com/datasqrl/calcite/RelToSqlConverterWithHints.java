...
public class RelToSqlConverterWithHints extends org.apache.calcite.rel.rel2sql.RelToSqlConverter {
  ...
  public SqlImplementor.Result visit(Project e) {
    ...
    builder.setHints(new SqlNodeList(hints, SqlParserPos.ZERO));
  ...
  public SqlImplementor.Result visit(Correlate e) {
    ...
    SqlNode rightLateral = SqlStdOperatorTable.LATERAL.createCall(POS, new SqlNode[]{rightResult.getNode()});
    SqlNode rightLateralAs = SqlStdOperatorTable.AS.createCall(POS, new SqlNode[]{rightLateral, new SqlIdentifier(rightResult.getNeededAlias(), POS)});
