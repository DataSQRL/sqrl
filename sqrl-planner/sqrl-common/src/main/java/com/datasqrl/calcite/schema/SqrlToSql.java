...
import com.datasqrl.calcite.schema.SqrlToSql.Context;
...
//      Preconditions.checkState(keySize == result.keysToPullUp.size());
  ...
  public Result visitTable(SqrlCompoundIdentifier node, Context context) {
    SqrlRelBuilder b = planner.getSqrlRelBuilder();
    ...
    if (item.getKind() == SqlKind.SELECT) {
      ...
      if (materializeSelf) {//treat self as a table
  ...
  public Result visitJoin(SqlJoin call, Context context) {
    ...
    Result rightNode = SqlNodeVisitor.accept(this, call.getRight(), context);
  ...
  public static class Context {
    ...
    List<String> currentPath;
    Map<String, List<String>> aliasPathMap = new HashMap<>();
