    ...
    SqlNodeList columnList =
        new SqlNodeList(
            ImmutableNullableList.of(
                new SqlIdentifier("column1", pos), new SqlIdentifier("column2", pos)),
            pos);
    SqlSelect select =
        new SqlSelectBuilder(SqlParserPos.ZERO)
            .setFrom(new SqlIdentifier("X", SqlParserPos.ZERO))
            .build();

    SqlCharStringLiteral comment =
        SqlLiteral.createCharString("This is an advanced view with multiple features.", pos);

    SqlCreateSnowflakeView createView =
        new SqlCreateSnowflakeView(
            pos, true, true, true, false, null, viewName, columnList, select, comment, true);
