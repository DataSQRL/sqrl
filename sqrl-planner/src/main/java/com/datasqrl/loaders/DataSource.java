...
public class DataSource {
  ...
  public Optional<TableSource> readTableSource(
      TableSchema tableSchema, TableConfig tableConfig, ErrorCollector errors, NamePath basePath) {
    ...
    if (tableSchema == null) {
      errors.warn(
          "Found configuration for table [%s] but no schema. Table not loaded.",
          tableConfig.getName());
      return Optional.empty();
    }
  ...
  public Optional<TableSink> readTableSink(
      Optional<TableSchema> schema, TableConfig tableConfig, NamePath basePath) {
