...
public abstract class AbstractJDBCEngine extends ExecutionEngine.Base implements ExecutionEngine {
  ...
  public AbstractJDBCEngine(
      @NonNull String name, @NonNull Type type, @NonNull EnumSet<EngineFeature> capabilities) {
  ...
  public DatabasePhysicalPlan plan(
    ...
    Optional<JdbcDDLFactory> optFactory = (new JdbcDDLServiceLoader()).load(getDialect());
    ...
    if (optFactory.isPresent()) {
      ...
      ddlStatements.addAll(
          StreamUtil.filterByClass(inputs, EngineSink.class)
    ...
    Map<IdentifiedQuery, QueryTemplate> databaseQueries =
        dbPlan.getQueries().stream()
            .collect(
                Collectors.toMap(
                    ReadQuery::getQuery,
                    q -> new QueryTemplate(getDialect().name().toLowerCase(), q.getRelNode())));
    ...
    for (Map.Entry<IdentifiedQuery, QueryTemplate> entry : databaseQueries.entrySet()) {
      ...
      SqlNode sqlNode =
          framework.getQueryPlanner().relToSql(Dialect.POSTGRES, relNode).getSqlNode();

      String viewSql = createView(viewNameIdentifier, pos, columnList, sqlNode);
      views.add(new DatabaseViewImpl(viewName, viewSql));
    }

    return new JDBCPhysicalPlan(ddlStatements, views, databaseQueries);
  }
  ...
  /**
   * If writing to the database required downcasting of types, this should return the corresponding
   * upcasting mapper to restore the original type.
   *
   * @return
   */
  ...
  private RelNode applyUpcasting(
      SqrlFramework framework,
    ...
    List<RexNode> fields =
        relNode.getRowType().getFieldList().stream()
            .map(
                field ->
                    convertField(framework, field, hasChanged, relBuilder, icebergDataTypeMapper))
            .collect(Collectors.toList());
  ...
  private RexNode convertField(
    ...
    framework
        ...
        .registerCatalogFunction(
            ...
            castFunction1.getFunction().getClass(),
            true);
    ...
    framework
        ...
        .lookupOperatorOverloads(
            new SqlIdentifier(
                castFunction1.getFunction().getClass().getSimpleName(), SqlParserPos.ZERO),
  ...
  private List<SqlDDLStatement> extractTypeExtensions(
      RelNode relNode, List<PgExtension> extensions) {
      ...
      for (PgExtension extension : extensions) {
        if (field.getType() instanceof RawRelDataType
            && ((RawRelDataType) field.getType()).getRawType().getOriginatingClass()
                == extension.typeClass()) statements.add(extension.getExtensionDdl());
      }
    }
          ...
          public RexNode visitCall(RexCall call) {
            for (PgExtension extension : extensions) {
              for (String function : extension.operators()) {
                if (function.equals(Name.system(call.getOperator().getName().toLowerCase()))) {
