...
public interface QueryEngine extends ExecutionEngine {
  ...
  @Override
  default DatabasePhysicalPlan plan(
      StagePlan plan,
    ...
    throw new UnsupportedOperationException(
        "Query Engine planning should be invoked through TableFormatEngine via the other plan"
            + " method");
  }

  DatabasePhysicalPlan plan(
      ConnectorFactoryFactory tableConnectorFactory,
      EngineConfig tableConnectorConfig,
      StagePlan plan,
      List<StageSink> inputs,
      ExecutionPipeline pipeline,
      List<StagePlan> stagePlans,
