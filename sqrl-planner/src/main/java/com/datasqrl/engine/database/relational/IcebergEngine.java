...
public class IcebergEngine extends AbstractJDBCTableFormatEngine {
  ...
  public DatabasePhysicalPlan plan(
    ...
    // TODO: Eventually we want to create the DDL statement for Iceberg to register in the catalog
    // independent
    // of the registered query engines. For now, we use the DDL from one of the query engines
    //    DatabasePhysicalPlan sinkPlan = super.plan(plan, inputs, pipeline, stagePlans, framework,
    // errorCollector);
    ...
    sinkDDL =
        queryEnginePlans.values().stream()
            .map(DatabasePhysicalPlan::getDdl)
            .findFirst()
            .orElse(List.of());
    ...
    //          Optional<IndexDefinition> optIndex =  dbPlan.getIndexDefinitions().stream().filter(i
    // -> i.getTableId().equals(tableId)).findFirst();
    //          if (optIndex.isPresent()) {
    //            IndexDefinition mainIndex = optIndex.get();
    //            System.out.println("Table: " + tableId);
    //            System.out.println("Partition columns: " + String.join(", ",
    // mainIndex.getColumnNames().subList(0, mainIndex.getPartitionOffset())));
    //            System.out.println("Sort columns: " + String.join(", ",
    // mainIndex.getColumnNames().subList(mainIndex.getPartitionOffset(),
    // mainIndex.getColumns().size())));
    //          } else {
