                ...
                option ->
                    new SqlTableOption(
                        ...
                        SqlLiteral.createCharString(
                            option.getValue().toString(), SqlParserPos.ZERO),
                        SqlParserPos.ZERO))
    ...
    for (RelDataTypeField column : fieldList) {
      ...
      if (metadataConfig.containsKey(columnName)) {
        ...
        if (metadataFnc instanceof SqlCall) {
          node =
              new SqlTableColumn.SqlComputedColumn(
                  SqlParserPos.ZERO, FlinkSqlNodeFactory.identifier(columnName), null, metadataFnc);
        } else {
          node =
