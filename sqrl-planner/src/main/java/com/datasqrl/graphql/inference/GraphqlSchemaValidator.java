...
public class GraphqlSchemaValidator extends SchemaWalker {
  ...
  @Inject
  public GraphqlSchemaValidator(SqrlFramework framework, APIConnectorManager apiConnectorManager) {
    this(framework.getCatalogReader().nameMatcher(), framework.getSchema(), apiConnectorManager);
  }

  public GraphqlSchemaValidator(
      SqlNameMatcher nameMatcher, SqrlSchema schema, APIConnectorManager apiManager) {
  ...
  protected void walkSubscription(
    ...
    checkState(
        functions.size() == 1, fieldDefinition.getSourceLocation(), "Cannot overload subscription");
    Function function = Iterables.getOnlyElement(functions);
    checkState(
        function instanceof SqrlTableMacro,
  ...
  protected void walkMutation(
    ...
    TableSource mutationSink =
        apiManager.getMutationSource(source, Name.system(fieldDefinition.getName()));
    ...
    validateStructurallyEqualMutation(
        fieldDefinition,
        getValidMutationReturnType(fieldDefinition, registry),
  ...
  private Object validateStructurallyEqualMutation(
      FieldDefinition fieldDefinition,
      ObjectTypeDefinition returnTypeDefinition,
    ...
    for (FieldDefinition returnTypeFieldDefinition : returnTypeDefinition.getFieldDefinitions()) {
      ...
      InputValueDefinition inputDefinition =
          findExactlyOneInputValue(fieldDefinition, name, inputType.getInputValueDefinitions());
  ...
  private void validateStructurallyEqualMutation(
      FieldDefinition fieldDefinition,
      InputValueDefinition inputDefinition,
      TypeDefinitionRegistry registry) {
    checkState(
        fieldDefinition.getName().equals(inputDefinition.getName()),
        fieldDefinition.getSourceLocation(),
  ...
  private Object validateStructurallyType(
      FieldDefinition field, Type definitionType, Type inputType, TypeDefinitionRegistry registry) {
    if (inputType instanceof NonNullType) {
      ...
      if (definitionType instanceof NonNullType) {
        ...
        return validateStructurallyType(
            field, nonNullDefinitionType.getType(), nonNullType.getType(), registry);
      } else {
      ...
      checkState(
          definitionType instanceof ListType,
          definitionType.getSourceLocation(),
      ...
      return validateStructurallyType(
          field, definitionListType.getType(), inputListType.getType(), registry);
    } else if (inputType instanceof TypeName) {
      // If subtype nonnull then it could return errors
      checkState(
          !(definitionType instanceof NonNullType),
          definitionType.getSourceLocation(),
      ...
      TypeDefinition defTypeDef =
          registry
              .getType(defTypeName)
              .orElseThrow(
                  () ->
                      createThrowable(
                          defTypeName.getSourceLocation(),
                          "Could not find type: %s",
                          defTypeName.getName()));
      ...
      if (inputTypeDef instanceof ScalarTypeDefinition) {
        checkState(
            defTypeDef instanceof ScalarTypeDefinition
                && inputTypeDef.getName().equals(defTypeDef.getName()),
            field.getSourceLocation(),
            "Scalar types not matching for field [%s]: found %s but wanted %s",
            field.getName(),
        ...
        checkState(
            defTypeDef instanceof EnumTypeDefinition
                || defTypeDef instanceof ScalarTypeDefinition
                    && inputTypeDef.getName().equals(defTypeDef.getName()),
            field.getSourceLocation(),
            "Enum types not matching for field [%s]: found %s but wanted %s",
            field.getName(),
            inputTypeDef.getName(),
            defTypeDef.getName());
        return null;
        ...
        checkState(
            defTypeDef instanceof ObjectTypeDefinition,
            field.getSourceLocation(),
            "Return object type must match with an input object type not matching for field [%s]:"
        ...
        return validateStructurallyEqualMutation(
            field, objectDefinition, inputDefinition, List.of(), registry);
      } else {
        throw createThrowable(
            inputTypeDef.getSourceLocation(),
    ...
    throw createThrowable(
        field.getSourceLocation(), "Unknown type encountered for field: %s", field.getName());
  }
  ...
  private InputObjectTypeDefinition getValidMutationInput(
    ...
    checkState(
        fieldDefinition.getInputValueDefinitions().get(0).getType() instanceof NonNullType,
        fieldDefinition.getSourceLocation(),
        "["
            + fieldDefinition.getName()
            + "] "
            + fieldDefinition.getInputValueDefinitions().get(0).getName()
            + "Must be non-null.");
    NonNullType nonNullType =
        (NonNullType) fieldDefinition.getInputValueDefinitions().get(0).getType();
    checkState(
  ...
  private ObjectTypeDefinition getValidMutationReturnType(
      FieldDefinition fieldDefinition, TypeDefinitionRegistry registry) {
    ...
    checkState(
        ...
        "[%s] must be a singular return value",
        fieldDefinition.getName());
    ...
    checkState(
        ...
        "Return must be an object type: %s",
        fieldDefinition.getName());
  ...
  protected void visitQuery(
    ...
    //          field.getSourceLocation(),
    //          "Cannot redefine a type to point to a different SQRL table. Use an interface
    // instead.\n"
    ...
    //        visitedObj.get(parentType) == null ? null :
    // visitedObj.get(parentType).getFullPath().getDisplay());
    //      }
    //    }
  ...
  public void validate(APISource source, ErrorCollector errors) {
    try {
      ...
      Optional<ObjectTypeDefinition> queryType =
          getType(registry, () -> getQueryTypeName(registry));
