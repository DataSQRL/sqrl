...
public abstract class SchemaWalker {
  ...
  private void walkSubscription(
      ObjectTypeDefinition m, TypeDefinitionRegistry registry, APISource source) {
    for (FieldDefinition fieldDefinition : m.getFieldDefinitions()) {
      walkSubscription(m, fieldDefinition, registry, source);
    }
  }

  protected abstract void walkSubscription(
      ObjectTypeDefinition m,
      FieldDefinition fieldDefinition,
      TypeDefinitionRegistry registry,
      APISource source);

  private void walkMutation(
      ObjectTypeDefinition m, TypeDefinitionRegistry registry, APISource source) {
    for (FieldDefinition fieldDefinition : m.getFieldDefinitions()) {
  ...
  protected abstract void walkMutation(
      APISource source,
      TypeDefinitionRegistry registry,
      ObjectTypeDefinition m,
      FieldDefinition fieldDefinition);
  ...
  public Object visitQuery(
    ...
    checkState(
        optType.isPresent(),
        field.getType().getSourceLocation(),
        "Could not find object in graphql type registry");

    // Let;s check for all the types here.
    checkState(
        optType.get() instanceof ObjectTypeDefinition,
