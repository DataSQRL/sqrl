...
/**
 * Expands table function calls for {@link QueryTableFunction} by substituting the parameters into
 * the query.
 */
public class DAGFunctionExpansionRule extends RelOptRule {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    QueryTableFunction tblFct =
        SqrlRexUtil.getCustomTableFunction(scan)
            .filter(
                fct ->
                    (fct
                        instanceof
                        QueryTableFunction)) // We want to preserve NestedRelationships and other
