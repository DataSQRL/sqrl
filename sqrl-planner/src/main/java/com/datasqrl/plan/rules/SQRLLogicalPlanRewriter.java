...
/**
 * The {@link SQRLLogicalPlanRewriter} rewrites the logical plan (i.e. {@link RelNode} produced by
 * the transpiler.
 *
 * <p>The logical plan rewriting serves a number of purposes:
 *
 * <ul>
 *   <li>Convert convenience features introduced by the transpiler (e.g. {@code DISTINCT ON}
 *       statements or nested limits) to proper Relnodes that can be processed by engines. The
 *       transpiler adds hints to the relnodes that this class expands to proper relational algebra
 *       (e.g. convert {@code DISTINCT ON} to a partitioned window-over with filter on row_number).
 *   <li>Keep track of primary key columns and timestamps which are needed to write row data to a
 *       database (i.e. to materialize data). This class attempts to infer the primary key and
 *       timestamp from the relational operator and expressions and pulls those columns through if
 *       the user does not explicitly select them.
 *   <li>Keep track of the {@link TableType} of each Relnode so that we can infer optimizations and
 *       likely user intent for ambiguous relational operators. For example, a default join between
 *       a stream and a state table on the state table's primary key is rewritten as a temporal join
 *       since that is the most likely intent of the user. If the user does not want a temporal
 *       join, they can explicitly declare the join to be an inner join. The {@link TableType} is
 *       also used in cost modeling and picking the right connectors. In addition, we keep track of
 *       potential time tumbling windows in {@link AnnotatedLP#timestamp} to infer when a user
 *       intents to execute a time windowed operation (e.g. window aggregation, join, or TopN). The
 *       overall goal is to rewrite "normal" SQL that users are likely to write into optimized
 *       stream processing SQL that uses time windows where possible.
 *   <li>Pull up relational operators that can be executed later in the pipeline/DAG without
 *       changing semantics. A key goal for DataSQRL is making it easier to materialize data in
 *       stream, but some operations like sorting or filtering on the current time are very
 *       expensive in the stream and cheap in the database (i.e. later in the pipeline/DAG). Hence,
 *       we try to identify and pull up such expensive operations to produce more optimal DAGs. See
 *       {@link PullupOperator} for the types of operators we pull up. For the same reason, we keep
 *       track of the {@link AnnotatedLP#streamRoot} when processing streams so we can determine
 *       when we are doing operations on a single stream record for nested data, so we can add a
 *       small time window for efficient on stream processing and avoid creating state.
 * </ul>
 *
 * We keep track of the metadata for a RelNode in the {@link AnnotatedLP} class. This class
 * implements a visitor pattern over RelNodes that creates the {@link AnnotatedLP} for each RelNode.
 * We try to process all RelNodes by computing the associated metadata and pullups in {@link
 * AnnotatedLP}. If that is not possible (e.g. certain set operations do not allow primary key
 * inference) we treat a RelNode as a {@link TableType#RELATION} and don't do any extra processing.
 */
public class SQRLLogicalPlanRewriter extends AbstractSqrlRelShuttle<AnnotatedLP> {
  ...
  SQRLLogicalPlanRewriter(
      @NonNull RelBuilder relBuilder,
      @NonNull SqrlConverterConfig config,
  ...
  protected RelNode setRelHolder(AnnotatedLP relHolder) {
    ...
    Preconditions.checkArgument(
        !relHolder.type.hasTimestamp() || !relHolder.timestamp.is(Timestamps.Type.UNDEFINED),
        "Timestamp required");
    Preconditions.checkArgument(
        !relHolder.type.hasPrimaryKey() || !relHolder.primaryKey.isUndefined(),
        "Primary key required");
    errors.checkFatal(
        relHolder.type != TableType.LOOKUP || result instanceof LogicalTableScan,
        "Lookup tables can only be used in temporal joins");
    return result;
  }
  ...
  private RelNode processRelation(List<AnnotatedLP> inputs, RelNode node) {
    ...
    List<RelNode> newInputs =
        inputs.stream()
            .map(alp -> alp.toRelation(makeRelBuilder(), exec))
            .map(AnnotatedLP::getRelNode)
  ...
  public RelNode visit(RelNode relNode) {
    ...
    List<AnnotatedLP> inputs =
        relNode.getInputs().stream()
            .map(input -> getRelHolder(input.accept(this)))
            .collect(Collectors.toUnmodifiableList());
  ...
  public RelNode visit(TableFunctionScan functionScan) {
    ...
    errors.checkFatal(
        tableFunctionOpt.isPresent(),
        "Invalid table function encountered in query: %s",
        functionScan);
    ...
    if (tableFunction instanceof NestedRelationship) {
      ...
      return setRelHolder(
          new AnnotatedLP(
              ...
              Optional.empty(),
              NowFilter.EMPTY,
              TopNConstraint.EMPTY,
              SortOrder.EMPTY,
              List.of()));
    } else if (tableFunction instanceof QueryTableFunction) {
      exec.requireFeature(
          EngineFeature.TABLE_FUNCTION_SCAN); // We only support table functions on the read side
  ...
  public RelNode visit(LogicalValues logicalValues) {
    ...
    return setRelHolder(
        AnnotatedLP.build(
                ...
                pk,
                Timestamps.UNDEFINED,
                SelectIndexMap.identity(numCols, numCols),
                List.of())
            .build());
  }
  ...
  /**
   * Determines the primary key of a table by rowtype using the simple heuristic of selecting the
   * first non-null scalar column.
   *
   * @param rowType
   * @return
   */
  ...
  public RelNode visit(LogicalFilter logicalFilter) {
    ...
    for (RexNode node : conjunctions) {
      ...
      if (idxOpt.isPresent() && input.relNode instanceof LogicalProject) {
        ...
        if (column instanceof RexOver && column.isA(SqlKind.ROW_NUMBER)) {
          newPk =
              ((RexOver) column)
                  .getWindow().partitionKeys.stream()
                      .map(
                          n ->
    ...
    if (FIND_NOW.foundIn(condition)) {
      ...
      while (iter.hasNext()) {
        ...
        if (FIND_NOW.foundIn(conj)) {
          Optional<TimePredicate> tp =
              TimePredicateAnalyzer.INSTANCE
                  .extractTimePredicate(
                      conj, rexUtil.getBuilder(), timestamp.isCandidatePredicate())
    ...
    if (!timeFunctions.isEmpty()) {
      ...
      errors.checkFatal(
          resultFilter.isPresent(), "Found one or multiple now-filters that cannot be satisfied.");
  ...
  public RelNode visit(LogicalProject logicalProject) {
    ...
    if (topNHintOpt.isPresent()) {
      ...
      baseInput =
          baseInput.inlineNowFilter(makeRelBuilder(), exec).inlineTopN(makeRelBuilder(), exec);
      ...
      List<Integer> partition =
          topNHint.getPartition().stream().map(baseInput.select::map).collect(Collectors.toList());

      Pair<SelectIndexMap, Boolean> trivialMapResult =
          getTrivialMapping(logicalProject, baseInput.select);
      Preconditions.checkArgument(
          trivialMapResult != null && !trivialMapResult.getRight(),
          "Select must be trivial and not rename");
      ...
      if (topNHint.getType() == TopNHint.Type.SELECT_DISTINCT) {
        List<Integer> distincts = SqrlRexUtil.combineIndexes(partition, select.targetsAsList());
        pk = PrimaryKeyMap.of(distincts);
        if (partition.isEmpty()) {
          // If there is no partition, we can ignore the sort order plus limit and turn this into a
        ...
        Optional<Integer> timestampOrder =
            LPConverterUtil.getTimestampOrderIndex(collation, timestamp);
        if (config.isFilterDistinctOrder()) {
          if (timestampOrder.isPresent()) {
            ...
            errors.checkFatal(
                baseInput.type.isStream(), "Filtered distinct only supported on streams");
            errors.checkFatal(
                collation.getFieldCollations().size() == 1,
                "Filtered distinct requires a single order column");
            RelFieldCollation fieldCol = collation.getFieldCollations().get(0);
            errors.checkFatal(
                fieldCol.direction.isDescending(), "Filtered distinct requires descending order");
            errors.checkFatal(
                timestamp.size() == 1,
                "Filtered distinct requires a single timestamp column, but found: %",
                timestamp); // TODO: replace with fieldname & index

            CalciteUtil.addFilteredDeduplication(
                relB, timestamp.getOnlyCandidate(), partition, fieldCol.getFieldIndex());
          }
        }
        if (baseInput.type.isStream()) {
          if (timestampOrder.isEmpty()) {
            errors.notice(
                DISTINCT_ON_TIMESTAMP, "DISTINCT ON statements is not ordered by timestamp");
          } else {
    ...
    Pair<SelectIndexMap, Boolean> trivialMapResult =
        getTrivialMapping(logicalProject, rawInput.select);
    ...
    // We only keep track of the first mapped project and consider it to be the "preserving one" for
    ...
    BiFunction<Integer, RelDataType, Integer> preserveIndex =
        (originalIndex, inputType) -> {
    ...
    for (Ord<RexNode> exp : Ord.<RexNode>zip(logicalProject.getProjects())) {
      ...
      if (inputRef.isPresent()) { // Direct mapping
        ...
        timestampBuilder.candidate(exp.i);
              ...
              TimestampAnalysis.extractTumbleWindow(
                  exp.i, mapRex, rexUtil.getBuilder(), input.timestamp))
          .isPresent()) {
        timeWindows.add(timeWindow.get());
      }
    }
    // Make sure we pull the primary keys through (i.e. append those to the projects if not already
    // present)
    ...
    List<RelFieldCollation> collations =
        input.sort.getCollation().getFieldCollations().stream()
            .map(
                collation ->
                    collation.withFieldIndex(
                        preserveIndex.apply(collation.getFieldIndex(), rowType)))
            .collect(Collectors.toUnmodifiableList());
    ...
    return setRelHolder(
        AnnotatedLP.build(
                ...
                primaryKey.build(),
                timestamp,
                SelectIndexMap.identity(logicalProject.getProjects().size(), fieldCount),
                input)
  ...
  private Pair<SelectIndexMap, Boolean> getTrivialMapping(
      LogicalProject project, SelectIndexMap baseMap) {
    ...
    for (Ord<RexNode> exp : Ord.<RexNode>zip(project.getProjects())) {
      ...
      if (!name.equals(inputNames.get(index))
          && !name.startsWith(ReservedName.SYSTEM_PRIMARY_KEY.getDisplay())) {
        isRenamed = true;
      }
  ...
  public RelNode visit(LogicalJoin logicalJoin) {
    ...
    JoinAnalysis joinAnalysis =
        JoinAnalysis.of(logicalJoin.getJoinType(), getJoinModifier(logicalJoin.getHints()));

    // We normalize the join by: 1) flipping right to left joins and 2) putting the stream on the
    // left for stream-on-x joins
    if (joinAnalysis.isA(Side.RIGHT)
        || (joinAnalysis.isA(Side.NONE)
            && !leftInput.type.isStream()
            && rightInput.type.isStream())) {
    ...
    SelectIndexMap joinedIndexMap =
        leftInput.select.join(rightInput.select, leftSideMaxIdx, joinAnalysis.isFlipped());
    List<RelDataTypeField> inputFields =
        ListUtils.union(
            leftInput.relNode.getRowType().getFieldList(),
    ...
    errors.checkFatal(!FIND_NOW.foundIn(condition), "now() is not allowed in join conditions");
    JoinConditionDecomposition eqDecomp = rexUtil.decomposeJoinCondition(condition, leftSideMaxIdx);
    ...
    if (joinAnalysis.canBe(Type.TEMPORAL)) {
      if (leftInput.type.isStream()
          && (rightInput.type == TableType.VERSIONED_STATE
              || rightInput.type == TableType.LOOKUP)) {
        ...
        if (isPKConstrained.get(Side.RIGHT) && rightInput.nowFilter.isEmpty()) {
          ...
          TemporalJoinHint hint = new TemporalJoinHint(joinTimestamp.getOnlyCandidate());
          ...
          return setRelHolder(
              AnnotatedLP.build(
                  ...
                  .streamRoot(leftInput.streamRoot)
                  .nowFilter(leftInput.nowFilter)
                  .sort(leftInput.sort)
                  .build());
        ...
        errors.fatal(
            "Expected %s side of the join to be stream and the other temporal state.",
    ...
    Function<Integer, RexInputRef> idxResolver =
        idx -> {
    ...
    SortOrder joinedSort =
        joinAnalysis.isFlipped() ? rightSort.join(leftSort) : leftSort.join(rightSort);

    // Detect interval join
    if (leftInputF.type.isStream() && rightInputF.type.isStream()) {
      // Validate that the join condition includes time bounds on the timestamp columns of both
      // sides of the join
      ...
      // Convert time-based predicates to normalized form for analysis and get the ones that
      // constrain both timestamps
      List<TimePredicate> timePredicates =
          conjunctions.stream()
              .map(
                  rex ->
                      TimePredicateAnalyzer.INSTANCE.extractTimePredicate(
                          rex, rexUtil.getBuilder(), isTimestampColumn))
              .flatMap(Optional::stream)
              .filter(tp -> !tp.hasTimestampFunction())
              // making sure predicate contains columns from both sides of the join
              .filter(
                  tp ->
                      (tp.getSmallerIndex() < leftSideMaxIdx)
                          ^ (tp.getLargerIndex() < leftSideMaxIdx))
              .collect(Collectors.toList());
      /*
      ...
      if (timePredicates.isEmpty()
          && leftInputF
              .streamRoot
              .filter(
                  left -> rightInputF.streamRoot.filter(right -> right.equals(left)).isPresent())
              .isPresent()) {
        ...
        List<PrimaryKeyMap.ColumnSet> rightRootPks =
            rightInputF.primaryKey.asSubList(numRootPks).stream()
        ...
        if (allRootsCovered) {
          // Change primary key to only include root pk once and add equality time condition because
          // timestamps must be equal
          TimePredicate eqCondition =
              new TimePredicate(
                  rightInputF.timestamp.getAnyCandidate() + leftSideMaxIdx,
                  leftInputF.timestamp.getAnyCandidate(),
                  SqlKind.EQUALS,
                  0);
          ...
          concatPkBuilder.addAll(
              rightInputF
                  .primaryKey
                  .asList()
                  .subList(numRootPks, rightInputF.primaryKey.getLength())
                  .stream()
                  .map(col -> col.remap(remapRightSide))
                  .collect(Collectors.toUnmodifiableList()));
      ...
      boolean leftSideSmaller =
          timePredicates.stream().anyMatch(tp -> tp.getSmallerIndex() < leftSideMaxIdx);
      boolean rightSideSmaller =
          timePredicates.stream().anyMatch(tp -> tp.getSmallerIndex() >= leftSideMaxIdx);
      ...
      if (isEquality || (leftSideSmaller && rightSideSmaller)) {
        Set<Integer> timestampIndexes =
            timePredicates.stream()
                .flatMap(tp -> tp.getIndexes().stream())
                .collect(Collectors.toSet());
        errors.checkFatal(
            timestampIndexes.size() == 2,
            WRONG_INTERVAL_JOIN,
            "Invalid interval condition - more than 2 timestamp columns used: %s",
            condition);
        ...
        Timestamps joinTimestamp =
            Timestamps.build(isEquality ? Timestamps.Type.OR : Timestamps.Type.AND)
                .candidates(timestampIndexes)
                .build();
        ;
        ...
        relB.join(
            joinAnalysis.export(),
            condition); // Can treat as "standard" inner join since no modification is necessary in
                        // physical plan
        ...
        return setRelHolder(
            AnnotatedLP.build(
                    ...
                    joinTimestamp,
                    joinedIndexMap,
                    List.of(leftInputF, rightInputF))
                .streamRoot(rootTable)
                .sort(joinedSort)
                .build());
      } else if (joinAnalysis.isA(Type.INTERVAL)) {
        errors.fatal(
            "Interval joins require time bounds on the timestamp columns in the join condition.");
      }
    } else if (joinAnalysis.isA(Type.INTERVAL)) {
      errors.fatal("Interval joins are only supported between two streams.");
    }
    ...
    new JoinCostHint(
            leftInputF.type, rightInputF.type, eqDecomp.getEqualities().size(), singletonSide)
        .addTo(relB);
    ...
    joinTimestamp.candidates(
        rightInputF.timestamp.getCandidates().stream()
            .map(remapRightSide::map)
            .collect(Collectors.toUnmodifiableSet()));
  ...
  public RelNode visit(LogicalCorrelate logicalCorrelate) {
    AnnotatedLP leftInput =
        getRelHolder(logicalCorrelate.getLeft().accept(this)).inlineTopN(makeRelBuilder(), exec);
    AnnotatedLP rightInput =
        getRelHolder(logicalCorrelate.getRight().accept(this)).inlineTopN(makeRelBuilder(), exec);
    ...
    List<RexNode> requiredNodes =
        logicalCorrelate.getRequiredColumns().asList().stream()
            .map(leftInput.select::map)
            .map(idx -> rexUtil.makeInputRef(idx, relB))
            .collect(Collectors.toList());
    relB.push(rightInput.relNode);
    relB.correlate(
        logicalCorrelate.getJoinType(), logicalCorrelate.getCorrelationId(), requiredNodes);
  ...
  private JoinModifier getJoinModifier(ImmutableList<RelHint> hints) {
    for (RelHint hint : hints) {
      if (hint.hintName.equals(JoinModifierHint.HINT_NAME)) {
        return JoinModifierHint.CONSTRUCTOR.fromHint(hint).getJoinModifier();
      }
    }
  ...
  public RelNode visit(LogicalUnion logicalUnion) {
    List<AnnotatedLP> rawInputs =
        logicalUnion.getInputs().stream()
            .map(
                in ->
                    getRelHolder(in.accept(this))
                        .inlineNowFilter(makeRelBuilder(), exec)
        ...
        rawInputs.stream()
            .map(
                meta ->
                    meta.copy()
                        .sort(SortOrder.EMPTY)
                        .build()) // We ignore the sorts of the inputs (if any) since they are
                                  // streams and we union them the default sort is timestamp
            .map(
                meta ->
                    meta.postProcess(
                        makeRelBuilder(),
                        ...
                        config,
                        false,
                        errors)) // The post-process makes sure the input relations are aligned and
                                 // timestamps are chosen (pk,selects,timestamps)
            .collect(Collectors.toList());

    // Calcite ensures that a union has the same select columns. We need to also ensure primary keys
    ...
    Preconditions.checkArgument(
        pk.isSimple(), "Primary key should be simple after post-processing");
    Timestamps timestamp = inputs.get(0).getTimestamp();
    SelectIndexMap select = inputs.get(0).select;
    Preconditions.checkArgument(
        timestamp.size() == 1, "Should have only one timestamp after post-processing");
    if (!inputs.stream()
        .allMatch(
            alp -> {
              errors.checkFatal(
                  alp.select.equals(select), "Input streams select different columns");
              if (!alp.primaryKey.equals(pk)) {
                errors.warn(
                    "Union of streams with different primary keys (both column names and positions"
                        + " must align): %s vs %s",
                    alp.getFieldNamesWithIndex(alp.primaryKey.asSimpleList()),
                    inputs.get(0).getFieldNamesWithIndex(pk.asSimpleList()));
                return false;
              }
              if (!alp.timestamp.equals(timestamp)) {
                errors.warn(
                    "Union of streams with different timestamps (both column names and positions"
                        + " must align): %s vs %s",
                    alp.getFieldNamesWithIndex(alp.timestamp.asList()),
                    inputs.get(0).getFieldNamesWithIndex(timestamp.asList()));
                return false;
              }
              ...
              for (Integer pkIndex : pk.asSimpleList()) {
                if (!fields.get(pkIndex).equals(alpFields.get(pkIndex))) {
                  errors.warn(
                      "Union of streams with different primary key column types: %s vs %s",
    ...
    IntStream.range(0, selectIndexes.size())
        .forEach(i -> selectNames.add(i, null)); // those names must be the same, just copy them
    // Add names for any added columns
    Stream.concat(
            IntStream.range(0, pk.getLength())
                .mapToObj(
                    pkNo ->
                        Pair.of(
                            ReservedName.SYSTEM_PRIMARY_KEY.suffix(String.valueOf(pkNo)),
                            pk.get(pkNo).getOnly())),
            timestamp.getCandidates().stream()
                .map(idx -> Pair.of(ReservedName.SYSTEM_TIMESTAMP, idx)))
        .filter(pair -> !selectIndexes.contains(pair.getRight()))
        .sorted(Comparator.comparing(Pair::getRight))
        .forEach(
            pair -> {
              selectIndexes.add(pair.getRight());
              selectNames.add(pair.getLeft().getDisplay());
            });

    // In the following, we can assume that the input relations are aligned because we
    // post-processed them
    RelBuilder relBuilder = makeRelBuilder();
  ...
  public RelNode visit(LogicalAggregate aggregate) {
    // Need to inline TopN before we aggregate, but we postpone inlining now-filter in case we can
    // push it through
    final AnnotatedLP input =
        getRelHolder(aggregate.getInput().accept(this)).inlineTopN(makeRelBuilder(), exec);
  ...
  public AnnotatedLP processAggregate(AnnotatedLP input, LogicalAggregate aggregate) {
    ...
    if (input.type == TableType.STREAM && input.streamRoot.isPresent()) {
      ...
      if (input.primaryKey.asList().subList(0, numRootPks).stream()
          .allMatch(col -> col.containsAny(groupByIdx))) {
    ...
    if (input.type == TableType.STREAM) {
      Optional<Timestamps.TimeWindow> timeWindow =
          findTimeWindowInGroupBy(groupByIdx, input.timestamp, input.relNode);
      if (timeWindow.isPresent()) {
        return handleTimeWindowAggregation(
            input, groupByIdx, aggregateCalls, targetLength, timeWindow.get());
      }
    }
    ...
    */
    Optional<TimestampAnalysis.MaxTimestamp> maxTimestamp =
        TimestampAnalysis.findMaxTimestamp(aggregateCalls, input.timestamp);

    // Check if we need to propagate timestamps
    if (input.type.isStream()) {
      return handleTimestampedAggregationInStream(
          aggregate, input, groupByIdx, aggregateCalls, maxTimestamp, targetLength);
    }
    ...
    List<AggregateCall> aggregateCalls =
        aggregate.getAggCallList().stream()
            .map(
                agg -> {
                  errors.checkFatal(
                      agg.getCollation().getFieldCollations().isEmpty(),
                      NOT_YET_IMPLEMENTED,
                      "Ordering within aggregations is not yet supported: %s",
                      agg);
                  errors.checkFatal(
                      agg.getCollation().getFieldCollations().isEmpty(),
                      NOT_YET_IMPLEMENTED,
                      "Filtering within aggregations is not yet supported: %s",
                      agg);
                  return agg.copy(
  ...
  private AnnotatedLP handleNestedAggregationInStream(
      AnnotatedLP input, List<Integer> groupByIdx, List<AggregateCall> aggregateCalls) {
    ...
    NowFilter nowFilter =
        input.nowFilter.remap(IndexMap.singleton(timestampIdx, timestamp.getOnlyCandidate()));

    return AnnotatedLP.build(
            relB.build(), TableType.STREAM, pkSelect.pk, timestamp, pkSelect.select, input)
        .streamRoot(input.streamRoot)
        .nowFilter(nowFilter)
        .build();
  }
  ...
  private Optional<Timestamps.TimeWindow> findTimeWindowInGroupBy(
    ...
    List<Timestamps.TimeWindow> windows =
        timestamp.getWindows().stream()
            .filter(w -> w.qualifiesWindow(groupByIdx))
            .collect(Collectors.toUnmodifiableList());
    if (windows.size() > 1) {
      errors.fatal(
          ErrorCode.NOT_YET_IMPLEMENTED,
          "Do not currently support grouping by " + "multiple time windows: [%s]",
          windows);
  ...
  private AnnotatedLP handleTimeWindowAggregation(
      AnnotatedLP input,
      List<Integer> groupByIdx,
      List<AggregateCall> aggregateCalls,
      int targetLength,
      Timestamps.TimeWindow window) {
    Preconditions.checkArgument(
        window instanceof Timestamps.SimpleTumbleWindow, "Expected simple tumble window");
    ...
    return AnnotatedLP.build(
            relB.build(), TableType.STREAM, pkSelect.pk, newTimestamp, pkSelect.select, input)
        .nowFilter(nowFilter)
        .build();
  }

  private AnnotatedLP handleTimestampedAggregationInStream(
    ...
    int timestampIdx =
        maxTimestamp
            .map(MaxTimestamp::getTimestampIdx)
            .orElse(inputTimestamp.getBestCandidate(relB));

    if (!input.nowFilter.isEmpty()
        && exec.supportsFeature(EngineFeature.STREAM_WINDOW_AGGREGATION)) {
      NowFilter nowFilter = input.nowFilter;
      // Determine timestamp, add to group-By and
      Preconditions.checkArgument(
          nowFilter.getTimestampIndex() == timestampIdx, "Timestamp indexes don't match");
      errors.checkFatal(
          !groupByIdx.contains(timestampIdx),
          "Cannot group on timestamp: %s",
          rexUtil.getFieldName(timestampIdx, input.relNode));
      ...
      errors.checkFatal(
          slideWidthMs > 0 && slideWidthMs < intervalWidthMs,
          "Invalid sliding window widths: %s - %s",
          intervalWidthMs,
          slideWidthMs);
      new SlidingAggregationHint(timestampIdx, intervalWidthMs, slideWidthMs).addTo(relB);

      TopNConstraint dedup =
          TopNConstraint.makeDeduplication(
              pkAndSelect.pk.asSimpleList(), timestamp.getOnlyCandidate());
      return AnnotatedLP.build(
              relB.build(),
              TableType.VERSIONED_STATE,
              pkAndSelect.pk,
              timestamp,
              pkAndSelect.select,
              input)
      ...
      return handleStandardAggregation(
          input, groupByIdx, aggregateCalls, maxTimestamp, targetLength);
    }
  }

  private AnnotatedLP handleStandardAggregation(
      AnnotatedLP input,
      List<Integer> groupByIdx,
      List<AggregateCall> aggregateCalls,
      Optional<TimestampAnalysis.MaxTimestamp> maxTimestamp,
    ...
    if (maxTimestamp.isPresent()) {
      ...
      AggregateCall maxTimestampAgg =
          rexUtil.makeMaxAggCall(
              bestTimestampIdx,
              ReservedName.SYSTEM_TIMESTAMP.getCanonical(),
              groupByIdx.size(),
              relB.peek());
    ...
    return AnnotatedLP.build(
            relB.build(), resultType, pkSelect.pk, resultTimestamp, pkSelect.select, input)
        .build();
  }
  ...
  private Pair<PkAndSelect, Timestamps> addTimestampAggregate(
      ...
      List<AggregateCall> aggregateCalls) {
    int targetLength = groupByIdx.size() + aggregateCalls.size();
    // if agg-calls return types are nullable because there are no group-by keys, we have to make
    // then non-null
    if (groupByIdx.isEmpty()) {
      aggregateCalls =
          aggregateCalls.stream()
              .map(agg -> CalciteUtil.makeNotNull(agg, relBuilder.getTypeFactory()))
              .collect(Collectors.toList());
    }
    ...
    PkAndSelect pkAndSelect = aggregatePkAndSelect(groupByIdx, groupByIdxTimestamp, targetLength);
    return Pair.of(pkAndSelect, timestamp);
  }

  private PkAndSelect aggregatePkAndSelect(List<Integer> originalGroupByIdx, int targetLength) {
    return aggregatePkAndSelect(
        originalGroupByIdx,
   ...
   * @param finalGroupByIdx The list of selected group by indexes to be used in the aggregate (must
   *     be sorted)
   * @param targetLength The number of columns of the aggregate operator
   * @return
   */
  private PkAndSelect aggregatePkAndSelect(
      List<Integer> originalGroupByIdx, List<Integer> finalGroupByIdx, int targetLength) {
    Preconditions.checkArgument(
        finalGroupByIdx.equals(finalGroupByIdx.stream().sorted().collect(Collectors.toList())),
        "Expected final groupByIdx to be sorted");
    Preconditions.checkArgument(
        originalGroupByIdx.stream().map(finalGroupByIdx::indexOf).allMatch(idx -> idx >= 0),
        "Invalid groupByIdx [%s] to [%s]",
        originalGroupByIdx,
        finalGroupByIdx);
    PrimaryKeyMap pk =
        PrimaryKeyMap.of(
            originalGroupByIdx.stream().map(finalGroupByIdx::indexOf).collect(Collectors.toList()));
    assert pk.getLength() == originalGroupByIdx.size() && pk.isSimple();

    ContiguousSet<Integer> additionalIdx =
        ContiguousSet.closedOpen(finalGroupByIdx.size(), targetLength);
    SelectIndexMap.Builder selectBuilder =
        SelectIndexMap.builder(pk.getLength() + additionalIdx.size());
  ...
  public RelNode visit(LogicalSort logicalSort) {
    ...
    if (limit.isPresent()) {
      Optional<Integer> timestampOrder =
          LPConverterUtil.getTimestampOrderIndex(newCollation, input.timestamp);
      TopNConstraint topN =
          new TopNConstraint(List.of(), false, newCollation, limit, timestampOrder.isPresent());
      if (limit.get() == 1) {
        ...
        result =
            input.copy().primaryKey(PrimaryKeyMap.none()).timestamp(timestamp).topN(topN).build();
      } else {
        result = input.copy().topN(topN).build();
      }
    } else {
