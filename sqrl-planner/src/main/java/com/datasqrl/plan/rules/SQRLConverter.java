...
public class SQRLConverter {
  ...
  public AnnotatedLP convert(
      final RelNode relNode, SqrlConverterConfig config, ErrorCollector errors) {
    SQRLLogicalPlanRewriter sqrl2sql = new SQRLLogicalPlanRewriter(relBuilder, config, errors);
  ...
  public TablePlan convert(PhysicalTable table, SqrlConverterConfig config, ErrorCollector errors) {
    ...
    if (table instanceof ProxyImportRelationalTable) {
      ...
      QueryRelationalTable queryTable =
          (table instanceof QueryTableFunction)
              ? ((QueryTableFunction) table).getQueryTable()
              : (QueryRelationalTable) table;
