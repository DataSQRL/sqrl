...
public class AnnotatedLP implements RelHolder {
  ...
  @NonNull public RelNode relNode;
  /* Metadata we keep track of for each RelNode */
  @NonNull public TableType type;
  @NonNull public PrimaryKeyMap primaryKey;
  @NonNull public Timestamps timestamp;
  @NonNull public SelectIndexMap select;

  /** Used to detect whether the operator applies to a single stream record so we can optimize */
  @Builder.Default @NonNull public Optional<PhysicalRelationalTable> streamRoot = Optional.empty();

  /* Operators we pull up */
  @Builder.Default @NonNull public NowFilter nowFilter = NowFilter.EMPTY; // Applies before topN

  @Builder.Default @NonNull
  public TopNConstraint topN = TopNConstraint.EMPTY; // Applies before sort

  @Builder.Default @NonNull public SortOrder sort = SortOrder.EMPTY;

  /** We keep track of the inputs for data lineage so we can track processing */
  @Builder.Default @NonNull public List<AnnotatedLP> inputs = List.of();

  public static AnnotatedLPBuilder build(
      RelNode relNode,
      TableType type,
  ...
  public List<String> getFieldNamesWithIndex(List<Integer> indexes) {
    ...
    return indexes.stream()
        .map(i -> alpFields.get(i).getName() + "[" + i + "]")
  ...
  public AnnotatedLP inlineTopN(RelBuilder relBuilder, ExecutionAnalysis exec) {
    ...
    if (topN.isDistinct()
        || (topN.hasPartition() && topN.hasLimit())) { // distinct or (hasPartition and hasLimit)
      ...
      if (topN.isDistinct()) {
        ...
        Set<Integer> excludedIndexes =
            topN.getCollation().getFieldCollations().stream()
                .map(RelFieldCollation::getFieldIndex)
                .collect(Collectors.toSet());
        excludedIndexes.addAll(topN.getPartition());
        List<Integer> remainingDistincts =
            primaryKey.asList().stream()
                .filter(col -> !col.containsAny(excludedIndexes))
                .map(col -> col.pickBest(inputType))
                .collect(Collectors.toList());
        remainingDistincts.stream()
            .map(
                idx ->
                    new RexFieldCollation(
                        RexInputRef.of(idx, inputType), Set.of(SqlKind.NULLS_LAST)))
      ...
      projects.add(
          rexUtil.createRowFunction(
              SqlStdOperatorTable.ROW_NUMBER, partitionKeys, fieldCollations));
      projectNames.add(Name.hiddenString("rownum"));
      int rowNumberIdx = projectIdx.size(),
          rankIdx = rowNumberIdx + 1,
          denserankIdx = rowNumberIdx + 2;
      if (topN.isDistinct()) {
        ...
        if (topN.hasLimit()) {
          projects.add(
              rexUtil.createRowFunction(
                  SqlStdOperatorTable.DENSE_RANK, partitionKeys, fieldCollations));
      ...
      if (topN.isDistinct()) {
        ...
        if (topN.hasLimit()) {
          conditions.add(
              SqrlRexUtil.makeWindowLimitFilter(
                  rexBuilder, topN.getLimit(), denserankIdx, windowType));
        }
      } else {
        conditions.add(
            SqrlRexUtil.makeWindowLimitFilter(
                rexBuilder, topN.getLimit(), rowNumberIdx, windowType));
      }
  ...
  public AnnotatedLP inlineNowFilter(RelBuilder relB, ExecutionAnalysis exec) {
    ...
    return copy().relNode(relB.build()).nowFilter(NowFilter.EMPTY).build();
  }
  ...
  public AnnotatedLP inlineSort(RelBuilder relB, ExecutionAnalysis exec) {
    ...
    return copy().relNode(relB.build()).sort(SortOrder.EMPTY).build();
  }
  ...
  public SortOrder getDefaultOrder(AnnotatedLP alp) {
    ...
    if (alp.getType().isStream()) {
      collations.add(
          new RelFieldCollation(
              alp.getTimestamp().getOnlyCandidate(),
  ...
  public AnnotatedLP toRelation(RelBuilder relB, ExecutionAnalysis exec) {
    ...
    return AnnotatedLP.build(
            ...
            SelectIndexMap.identity(
                inlined.select.getSourceLength(), inlined.select.getSourceLength()),
            inlined)
        .build();
  }
   ...
   *
   * <p>Finalizes the RelNode by adding a projection for all the selected columns plus inferred
   * primary key and timestamp (if any) as well as sort orders. This preserves the order of the
   * select fields and may add additional fields at the end for primary key and timestamp metadata
   * as well as columns needed in the pulled up sort.
   *
   * <p>Note, that we pick a single timestamp even when there are multiple viable candidates to
   * ensure that a table has a single timestamp. We either pick the first viable timestamp the user
   * selected or the best candidate according to {@link Timestamps#getBestCandidate(RelBuilder)}.
   *
   * @return An updated {@link AnnotatedLP} with an additional projection.
   */
  public AnnotatedLP postProcess(
      @NonNull RelBuilder relBuilder,
      RelNode originalRelNode,
      SqrlConverterConfig config,
      boolean inlinePullups,
    ...
    Function<Integer, RelDataTypeField> getField =
        i -> relBuilder.peek().getRowType().getFieldList().get(i);
    ...
    if (originalRelNode != null
        && SqrlHint.fromRel(originalRelNode, TopNHint.CONSTRUCTOR)
            .filter(topN -> topN.getType() == TopNHint.Type.DISTINCT_ON)
            .isEmpty()) {
      ...
      nameAdjuster =
          new NameAdjuster(
              input.select.targetsAsList().stream().map(getFieldName).collect(Collectors.toList()));
    }
    Preconditions.checkArgument(originalFieldNames.size() == input.select.getSourceLength());

    Function<Integer, String> makeMeta =
        i -> nameAdjuster.uniquifyName(Name.hiddenString(getFieldName.apply(i)));
    ...
    if (config.primaryKeyNames != null) {
      ...
      for (String pkName : config.primaryKeyNames) {
        OptionalInt pkIndex =
            IntStream.range(0, projectNames.size())
                .filter(i -> projectNames.get(i).equalsIgnoreCase(pkName))
                .findFirst();
      ...
      for (int i = 0; i < input.primaryKey.getLength(); i++) {
        ...
        int[] newPkIndexes =
            IntStream.range(0, projectIndexes.size())
                .filter(idx -> columnSet.contains(projectIndexes.get(idx)))
                .toArray();
        List<Integer> oldPkIndexes =
            Arrays.stream(newPkIndexes).mapToObj(projectIndexes::get).collect(Collectors.toList());
        if (newPkIndexes.length > 1) {
          errors.notice(
              MULTIPLE_PRIMARY_KEY,
              "A primary key column is mapped to multiple columns in query: %s",
    ...
    // Determine which timestamp candidates have already been mapped and map the candidates
    // accordingly
    ...
    List<RexNode> projects =
        projectIndexes.stream()
            .map(
                idx ->
                    idx == PK_LITERAL_IDENTIFIER
                        ? relBuilder.literal(1)
                        : RexInputRef.of(idx, relBuilder.peek().getRowType()))
            .collect(Collectors.toUnmodifiableList());

    IndexMap remap = IndexMap.of(projectIndexes);
    SelectIndexMap updatedSelect =
        SelectIndexMap.identity(input.select.getSourceLength(), input.select.getSourceLength());
    ...
    if (!projectIndexes.equals(ContiguousSet.closedOpen(0, peekedFieldNames.size()).asList())
        || IntStream.range(0, peekedFieldNames.size())
            .filter(i -> projectNames.get(i) != null)
    ...
    return new AnnotatedLP(
        ...
        updatedSelect,
        input.streamRoot,
        input.nowFilter.remap(remap),
        input.topN.remap(remap),
        input.sort.remap(remap),
        List.of(this));
  }
