...
public class TableConverter {
  ...
  public SourceTableDefinition sourceToTable(
    ...
    if (dataType == null) {
      throw errors.exception(
          ErrorCode.SCHEMA_ERROR, "Could not convert schema for table: %s", tableName);
    }
    ...
    for (String columnName : metadataConfig.getKeys()) {
      ...
      errors.checkFatal(
          !nameAdjuster.contains(columnName),
          "Metadata column name already used in data: %s",
          columnName);
      MetadataEntry colConfig = metadataConfig.getMetadataEntry(columnName).get();
      ...
      if (type.isPresent()) { // if has a type, use that, otherwise resolve as module
        if (!isValidDatatype(type.get())) {
          throw new RuntimeException(
              "Not a valid SQRL data type. Please check the documentation for supported SQRL"
                  + " types.");
        }
        String datatype = type.get();
        RelDataType metadataType =
            planner
                .getRelBuilder()
                .getTypeFactory()
        ...
        if (sqlNode instanceof SqlCall) {
          ...
          try {
            RexNode rexNode =
                framework.getQueryPlanner().planExpression(sqlNode, typeBuilder.build());

            typeBuilder.add(nameAdjuster.uniquifyName(columnName), rexNode.getType());
          } catch (Exception e) {
            throw new RuntimeException(
                String.format(
                    "Could not evaluate metadata expression: %s. Reason: %s",
                    attribute, e.getMessage()));
          }
        } else if (sqlNode instanceof SqlIdentifier) {
          RelDataType relDataType = typeBuilder.build();

          RelDataTypeField field = relDataType.getField(attribute, false, false);
          if (field == null) {
            throw new RuntimeException(
                "Could not find metadata field:" + ((SqlIdentifier) sqlNode).getSimple());
          }
    ...
    List<String> primaryKeys =
        baseTblConfig
            .getPrimaryKey()
            //        .validate(list -> list!=null && !list.isEmpty(), "Need to specify a primary
            // key to unique identify records in table")
            //        .validate(list -> list.stream().allMatch(nameAdjuster::contains),
            //            String.format("Primary key column not found. Must be one of: %s",
            // nameAdjuster))
            .get();
    ...
    Preconditions.checkState(
        baseTblConfig.getTimestampColumn().isPresent(), "timestamp column missing");
    String timestampColumn = baseTblConfig.getTimestampColumn().get();
    if (!nameAdjuster.contains(timestampColumn)) {
      throw new RuntimeException(
          String.format(
              "Timestamp column not found: \"%s\". Must be one of: %s",
              timestampColumn, nameAdjuster));
    }
    ...
    return new SourceTableDefinition(
        finalType,
        new PrimaryKey(pkIndexes),
        baseTblConfig.getTimestampColumn().map(col -> getFieldIndex(finalType, col)),
        tableType);
  }
  ...
  private void addModules(
    ...
    for (NamePath attribute : fncModules) {
      ...
      Optional<NamespaceObject> namespaceObject =
          moduleOpt.get().getNamespaceObject(Name.system(name));
