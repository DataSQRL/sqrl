...
public abstract class ScriptRelationalTable extends AbstractRelationalTable
   ...
   */
  @Getter protected int numSelects;

  @NonNull private final List<AddedColumn> addedColumns = new ArrayList<>();

  /** When a table is locked, no more columns can be added */
  @Getter private boolean isLocked;

  protected ScriptRelationalTable(
      @NonNull Name nameId, @NonNull RelDataType rowType, int numSelects) {
    super(nameId);
  ...
  public int addColumn(@NonNull AddedColumn column, @NonNull RelDataTypeFactory typeFactory) {
    ...
    rowType =
        CalciteUtil.addField(rowType, index, column.getNameId(), column.getDataType(), typeFactory);
    return index;
  }
