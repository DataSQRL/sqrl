...
public class CalciteTableFactory {
  ...
  public CalciteTableFactory(SqrlFramework framework, ModuleLoader moduleLoader) {
    this(
        new TableIdFactory(framework.getSchema().getTableNameToIdMap()),
        new TableConverter(framework.getTypeFactory(), framework));
  }

  public ImportedRelationalTableImpl createImportedTable(
      RelDataType rootType, TableSource tableSource, Name tableName) {
  ...
  public ProxyImportRelationalTable createProxyTable(
      RelDataType rootType,
      NamePath tablePath,
