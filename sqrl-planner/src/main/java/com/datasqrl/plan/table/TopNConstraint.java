...
public class TopNConstraint implements PullupOperator {
  ...
  public static TopNConstraint EMPTY =
      new TopNConstraint(List.of(), false, RelCollations.EMPTY, Optional.empty(), false);

  @NonNull List<Integer> partition; // First, we partition in the input relation
  boolean
      distinct; // second, we select distinct rows if true [All pk columns not in the partition
                // indexes are SELECT DISTINCT columns]
  ...
  public TopNConstraint(
      List<Integer> partition,
      boolean distinct,
      RelCollation collation,
      Optional<Integer> limit,
      boolean isTimestampOrder) {
  ...
  public TopNConstraint remap(IndexMap map) {
    ...
    List<Integer> newPartition =
        partition.stream().map(i -> map.map(i)).collect(Collectors.toList());
    return new TopNConstraint(newPartition, distinct, newCollation, limit, isTimestampOrder);
  }

  public List<Integer> getIndexes() {
    return Stream.concat(
            collation.getFieldCollations().stream().map(c -> c.getFieldIndex()), partition.stream())
        .collect(Collectors.toList());
  }

  public static TopNConstraint makeDeduplication(
      List<Integer> partitionByIndexes, int timestampIndex) {
    RelCollation collation =
        RelCollations.of(
    ...
    return new TopNConstraint(partitionByIndexes, false, collation, Optional.of(1), true);
  }

  public boolean isDeduplication() {
    // Deduplication may not have a partition for global/single row tables, e.g. after a global
    // aggregation
  ...
  public boolean isDescendingDeduplication() {
    ...
    return isDeduplication()
        && collation.getFieldCollations().get(0).getDirection()
            == RelFieldCollation.Direction.DESCENDING
