...
public class QueryRelationalTable extends PhysicalRelationalTable {
  ...
  @Getter private final Optional<PhysicalRelationalTable> streamRoot;

  public QueryRelationalTable(
      Name rootTableId, NamePath tablePath, @NonNull LPAnalysis analyzedLP) {
    super(
        rootTableId,
        tablePath,
    ...
    Preconditions.checkArgument(
        analyzedLP.getConvertedRelnode().select.isIdentity(), "We assume an identity select");
  ...
  public List<ExecutionStage> getSupportedStages(
    ...
    return analyzedLP.getConfiguredStages().stream()
        .map(
            stage -> {
              Optional<ExecutionStage> execStage =
                  pipeline
                      .getStage(stage.getStageName())
                      .or(() -> pipeline.getStageByType(stage.getStageName()));
              errors.checkFatal(
                  execStage.isPresent(),
                  "Could not find execution stage [%s] "
                      + "specified in optimizer hint on table [%s] in pipeline [%s]",
                  stage.getStageName(),
                  this,
                  pipeline);
  ...
  public SqrlConverterConfig.SqrlConverterConfigBuilder getBaseConfig() {
    SqrlConverterConfig.SqrlConverterConfigBuilder builder =
        analyzedLP.getConverterConfig().toBuilder();
