...
public class TimestampAnalysis {
  ...
  public static boolean computesTimestamp(@NonNull RexNode rexNode, @NonNull Timestamps timestamp) {
    ...
    if (operator.getKind().equals(SqlKind.CASE)) {
      Preconditions.checkArgument(
          operands.size() >= 3 && operands.size() % 2 == 1, "Invalid operand list: %s", operands);
  ...
  public static Optional<Timestamps.TimeWindow> extractTumbleWindow(
      int index, RexNode rexNode, RexBuilder rexBuilder, Timestamps timestamps) {
    if (!(rexNode instanceof RexCall)) {
      return Optional.empty();
    }
    RexCall call = (RexCall) rexNode;
    Optional<SqrlTimeTumbleFunction> fnc =
    ...
    Preconditions.checkArgument(
        call.getOperands().size() > 0, "Tumble window function must have at least one argument");
    ...
    for (int i = 1; i < call.getOperands().size(); i++) {
      ...
      Preconditions.checkArgument(
          RexUtil.isConstant(operand), "All non-timestamp arguments must be constants");
    ...
    if (!operands.isEmpty()) {
      ...
      operandValues =
          reducer.reduce2Long(
              rexBuilder, operands); // throws exception if arguments cannot be reduced
    }
    SqrlTimeTumbleFunction.Specification spec = tumbleFct.getSpecification(operandValues);
    return Optional.of(
        new Timestamps.SimpleTumbleWindow(
            index, timestampIdx, spec.getWindowWidthMillis(), spec.getWindowOffsetMillis()));
  }
  ...
  public static Optional<MaxTimestamp> findMaxTimestamp(
      List<AggregateCall> aggregateCalls, Timestamps timestamp) {
