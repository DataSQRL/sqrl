...
public class DAGAssembler {
  ...
  public PhysicalDAGPlan assemble(SqrlDAG dag, Set<URL> jars) {
    ...
    dag.allNodesByClass(SqrlDAG.QueryNode.class)
        .forEach(
            query -> {
              ExecutionStage stage = query.getChosenStage();
              AnalyzedAPIQuery apiQuery = query.getQuery();
              if (stage.getEngine().getType()
                  == Type.SERVER) { // this must be the serverStage by assumption
                RelNode relNode = apiQuery.getRelNode(serverStage.get(), sqrlConverter, errors);
                relNode =
                    RelStageRunner.runStage(
                        SERVER_DAG_STITCHING, relNode, framework.getQueryPlanner().getPlanner());
    ...
    Stream.concat(
            serverScanVisitor.scanTables.stream().map(DatabaseQuery::of),
    ...
    for (ExecutionStage database :
        Iterables.filter(pipeline.getStages(), queriesByStage::containsKey)) {
      ...
      queriesByStage.get(database).stream()
          .sorted(Comparator.comparing(DatabaseQuery::getName))
          .forEach(
              query -> {
                RelNode relNode = query.getRelNode(database, sqrlConverter, errors);
                relNode =
                    RelStageRunner.runStage(
                        DATABASE_DAG_STITCHING, relNode, framework.getQueryPlanner().getPlanner());
                tableScanVisitor.findScans(relNode);
                databaseQueries.add(new PhysicalDAGPlan.ReadQuery(query.getQueryId(), relNode));
              });

      Preconditions.checkArgument(
          tableScanVisitor.scanFunctions.isEmpty(),
          "Should not encounter table functions in materialized queries");
      List<PhysicalRelationalTable> materializedTables =
          tableScanVisitor.scanTables.stream().sorted().collect(Collectors.toList());
      ...
      for (PhysicalRelationalTable materializedTable : materializedTables) {
        List<String> nullablePks =
            CalciteUtil.identifyNullableFields(
                materializedTable.getRowType(), materializedTable.getPrimaryKey().asList());
        errors.checkFatal(
            nullablePks.isEmpty(),
            PRIMARY_KEY_NULLABLE,
            "Cannot materialize table [%s] with nullable primary key: %s",
            materializedTable,
            nullablePks);
        errors.checkFatal(
            materializedTable.getPrimaryKey().isDefined(),
            ErrorCode.TABLE_NOT_MATERIALIZE,
            "Table [%s] does not have a primary key and can therefore not be materialized",
            materializedTable);
        Preconditions.checkArgument(
            materializedTable.getTimestamp().size() <= 1,
            "Should not have multiple timestamps at this point");
        errors.checkFatal(
            materializedTable.getType() == TableType.STATIC
                || materializedTable.getTimestamp().size() == 1,
            ErrorCode.TABLE_NOT_MATERIALIZE,
            "Table [%s] does not have a timestamp and can therefore not be materialized",
            materializedTable);

        OptionalInt timestampIdx = materializedTable.getTimestamp().getOnlyCandidateOptional();
        RelNode processedRelnode =
            produceWriteTree(materializedTable.getPlannedRelNode(), timestampIdx);
        streamQueries.add(
            new PhysicalDAGPlan.WriteQuery(
                new EngineSink(
                    materializedTable.getNameId(),
                    materializedTable.getPrimaryKey().getPkIndexes(),
                    materializedTable.getRowType(),
                    timestampIdx,
                    database),
                processedRelnode,
                materializedTable.getPlannedRelNode(),
                materializedTable.getType()));
      }
              ...
              indexSelector
                  ...
                  .ifPresent(
                      indexHints -> {
                        ...
                        indexDefinitions.removeIf(
                            idx -> idx.getTableId().equals(table.getNameId()));
                        // and overwrite with the specified ones
                        indexDefinitions.addAll(indexHints);
                      }));
      databasePlans.add(
          new PhysicalDAGPlan.DatabaseStagePlan(database, databaseQueries, indexDefinitions));
    }

    // Add exported tables
    dag.allNodesByClass(ExportNode.class)
        .forEach(
            exportNode -> {
              ...
              RelNode processedRelnode =
                  produceWriteTree(
                      export.getRelNode(),
                      getExportBaseConfig().withStage(exportNode.getChosenStage()),
                      errors);
              ...
              if (export.getNumSelects().isPresent()) {
                ...
                Preconditions.checkArgument(
                    relBuilder1.peek().getRowType().getFieldCount() >= numFields2Select);
              ...
              ExternalSink externalSink =
                  new ExternalSink(exportNode.getUniqueId(), export.getSink());
              streamQueries.add(
                  new PhysicalDAGPlan.WriteQuery(
                      externalSink,
                      processedRelnode,
    ...
    //        .map(SqrlDAG.TableNode::getTable).filter(tbl ->
    // debugger.isDebugTable(tbl.getTableName()))
    //        .sorted(Comparator.comparing(PhysicalTable::getTableName))
    //        .forEach(table -> {
    //          Name debugSinkName = table.getTableName().suffix("debug" +
    // debugCounter.incrementAndGet());
    //          TableSink sink = debugger.getDebugSink(debugSinkName, errors);
    //          RelNode expandedRelNode = produceWriteTree(table.getPlannedRelNode(),
    // table.getTimestamp().getOnlyCandidate());
    ...
    PhysicalDAGPlan.StagePlan streamPlan =
        new StreamStagePlan(streamStage, streamQueries, streamTables, jars);
    ...
    if (serverStage.isPresent()) {
      PhysicalDAGPlan.StagePlan serverPlan =
          new PhysicalDAGPlan.ServerStagePlan(serverStage.get(), serverQueries);
    ...
    if (logStage.isPresent()) {
      List<Log> sortedLogs =
          logs.entrySet().stream()
              ...
              .collect(Collectors.toList());
      PhysicalDAGPlan.StagePlan logPlan =
          new PhysicalDAGPlan.LogStagePlan(logStage.get(), sortedLogs);
  ...
  private RelNode produceWriteTree(
      RelNode relNode, SqrlConverterConfig config, ErrorCollector errors) {
  ...
  private RelNode produceWriteTree(RelNode convertedRelNode, OptionalInt timestampIndex) {
    RelNode expandedRelNode =
        RelStageRunner.runStage(
            STREAM_DAG_STITCHING, convertedRelNode, framework.getQueryPlanner().getPlanner());
  ...
  private static class VisitTableScans extends RelShuttleImpl {
    ...
    public RelNode visit(TableFunctionScan scan) {
      SqrlRexUtil.getCustomTableFunction(scan)
          .filter(QueryTableFunction.class::isInstance)
          .map(QueryTableFunction.class::cast)
          .ifPresent(scanFunctions::add);
      return super.visit(scan);
    }
  }
}
