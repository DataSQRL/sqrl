...
public interface DatabaseQuery {
  ...
  static DatabaseQuery.Instance of(AbstractRelationalTable table) {
    Preconditions.checkArgument(
        table instanceof PhysicalRelationalTable, "Expected physical table");
    ...
    // TODO: We don't yet support server queries directly against materialized tables. Need a
    // database stage in between.
    Preconditions.checkArgument(
        stage.getEngine().getType() == Type.DATABASE,
        "We do not yet support queries directly against stream");
    return new Instance(
        vTable.getNameId(), vTable.getTableName(), List.of(), vTable.getPlannedRelNode(), stage);
  }
  ...
  static DatabaseQuery.Instance of(QueryTableFunction function) {
    ...
    return new Instance(
        queryTable.getNameId(),
        queryTable.getTableName(),
        function.getParameters(),
        queryTable.getPlannedRelNode(),
        assignedStage);
  }
  ...
  class Instance implements DatabaseQuery, IdentifiedQuery {
    ...
    public RelNode getRelNode(
        ExecutionStage stage, SQRLConverter sqrlConverter, ErrorCollector errors) {
      Preconditions.checkArgument(
          assignedStage.equals(stage),
          "Mismatch in stage: %s (assigned) vs %s (requested)",
          assignedStage,
          stage);
