...
public class SqrlDAGExporter {
                ...
                .primary_key(
                    table.getPrimaryKey().isUndefined()
                        ...
                        : table.getPrimaryKey().asList().stream()
                            ...
                            .collect(Collectors.toUnmodifiableList()))
                .timestamp(
                    table.getTimestamp().is(Timestamps.Type.UNDEFINED)
                        ? "-"
                        : fields.get(table.getTimestamp().getOnlyCandidate()).getName())
                .schema(
                    fields.stream()
                        .map(
                            field ->
  ...
  private static List<PostProcessor> convert(
      PullupOperator.Container pullups, List<RelDataTypeField> fields) {
    ...
    if (!pullups.getNowFilter().isEmpty()) {
      ...
      processors.add(
          new PostProcessor(
              "now-filter",
              fields.get(predicate.getLargerIndex()).getName()
                  + " > now() - "
                  + predicate.getIntervalLength()
                  + " ms"));
    }
    if (!pullups.getTopN().isEmpty()) {
      ...
      if (topN.hasPartition()) {
        description +=
            "partition="
                + topN.getPartition().stream()
                    .map(i -> fields.get(i).getName())
    ...
    if (!pullups.getSort().isEmpty()) {
      processors.add(
          new PostProcessor("sort", collations2String(pullups.getSort().getCollation(), fields)));
    }
    return processors;
  }
  ...
  private static String collations2String(RelCollation collation, List<RelDataTypeField> fields) {
    ...
    return collations.stream()
        .map(col -> fields.get(col.getFieldIndex()) + " " + col.shortString())
        .collect(Collectors.joining(", "));
  }
    ...
    public static NodeType from(TableType tableType) {
      switch (tableType) {
        ...
        case VERSIONED_STATE:
          return STATE;
        default:
          throw new UnsupportedOperationException("Unexpected type: " + tableType);
      }
    }
  }
