...
public abstract class ExecutionStageException extends RuntimeException {
    ...
    public static class StageChange extends ExecutionStageException {
        ...
        public static StageChange of(ExecutionStage start, ExecutionStage end) {
            return new StageChange(String.format("Processing relation requires stage change from [%s] to [%s]"
    ...
    public static class StageFinding extends ExecutionStageException {
        ...
        public static StageFinding of(ExecutionStage baseStage, Collection<EngineCapability> capabilities) {
            return new StageFinding(String.format("Could not find stage has capabilities [%s] starting at stage [%s]",
                    capabilities, baseStage.getName()));
        }

        public static StageFinding of(ExecutionStage baseStage, ExecutionStage other) {
            return new StageFinding(String.format("Could not find stage that combines [%s] starting at [%s]",
    ...
    public static class NoStage extends RuntimeException {
        ...
        public String getMessage() {
            ...
            s.append("Could not find stage that could process table:").append(relationName.getDisplay()).append("\n");
            for (Map.Entry<ExecutionStage, ExecutionStageException> entry : stageExceptions.entrySet()) {
                s.append(entry.getKey().getName()).append(" -> ").append(entry.getValue().getMessage()).append("\n");
            }
            return s.toString();
        }
