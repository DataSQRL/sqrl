...
public class SqrlRelMdRowCount extends RelMdRowCount
    ...
    public Double getRowCount(RelNode rel, RelMetadataQuery mq) {
        ...
        if (rel instanceof Filter) return getRowCount((Filter)rel,mq);
    ...
    public static Double getRowCount(VirtualRelationalTable table, List<IndexCall.IndexColumn> constraints) {
        Set<Integer> equalCols = constraints.stream().filter(c -> c.getType()== IndexCall.CallType.EQUALITY)
