...
@Slf4j
public class FileDataSystemDiscovery extends DataSystemDiscovery.Base { //remove
...
//    FormatFactoryOld format = tableConfig.getFormat();
...
//    return isTableFile(file, baseConfig, format, fileConfig);
  ...
  public static boolean isTableFile(@NonNull FilePath file, @NonNull BaseTableConfig baseConfig,
                                    @NonNull FormatFactoryOld format, @NonNull FileDataSystemConfig fileConfig) {
      ...
      for (FilePath.Status fps : pathConfig.listFiles()) {
        ...
        if (fps.isDir()) {
          ...
          if (componentsOpt.map(c -> Name.validName(c.getIdentifier())).orElse(false)) {
            ...
            Optional<FormatFactoryOld> extensionFormat = FileFormatExtension.getFormat(
                components.getFormat());
            FormatFactoryOld chosenFormat = null;
            SqrlConfig formatConfig = tblBuilder.getFormatConfig();
            if (genericTable.hasFormat()) {
              chosenFormat = genericTable.getFormat().get();
              ...
              formatConfig.setProperty(FormatFactoryOld.FORMAT_NAME_KEY, chosenFormat.getName());
            } else {
            ...
            if (otherTbl == null) {
              ...
              FormatFactoryOld otherFormat = otherTbl.getFormat().get();
