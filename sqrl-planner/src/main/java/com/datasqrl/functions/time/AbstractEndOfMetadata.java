...
public abstract class AbstractEndOfMetadata implements SqrlTimeTumbleFunction {
  ...
  public Specification getSpecification(long[] arguments) {
    ...
    return new Specification(
        arguments.length > 0 ? arguments[0] : 1, arguments.length > 1 ? arguments[1] : 0);
  }
  ...
  //            + "<br />An optional second argument specifies the time window width as multiple %s,
  // e.g. if the argument is 5 the function returns the end of the next 5 %s. 1 is the default."
  //            + "<br />An optional third argument specifies the time window offset in %s, e.g. if
  // the argument is 2 the function returns the end of the time window offset by 2 %s",
