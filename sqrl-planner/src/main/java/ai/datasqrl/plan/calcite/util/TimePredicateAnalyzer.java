...
public class TimePredicateAnalyzer {
    ...
    private Pair<Set<Integer>, RexNode> extractReferencesAndReplaceWithZero(RexNode rexNode, RexBuilder rexBuilder) {
        ...
        if (rexNode instanceof RexCall) {
            RexCall call = (RexCall) rexNode;
            if (SqrlFunction.unwrapSqrlFunction(call.getOperator()).filter(op -> op instanceof StdTimeLibraryImpl.NOW).isPresent()) {
                return Pair.of(Set.of(TimePredicate.NOW_INDEX), rexBuilder.makeZeroLiteral(rexNode.getType()));
            } else {
                //Map recursively
