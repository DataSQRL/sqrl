...
/**
 * Copied over from Flink to fix RAW type cast.
 *
 * <p>TODO: remove this when we update to Flink 2.2 or higher
 */
@Internal
public final class TypeInferenceOperandChecker
  ...
  public List<String> paramNames() {
    return typeInference
        .getStaticArguments()
        .map(args -> args.stream().map(StaticArgument::getName).collect(Collectors.toList()))
        .orElseThrow(
            () ->
                new ValidationException(
                    "Unsupported function signature. "
  ...
  private SqlDataTypeSpec createRawDataTypeSpec(RawRelDataType type) {
    ...
    SqlNode className =
        SqlLiteral.createCharString(rawType.getOriginatingClass().getName(), SqlParserPos.ZERO);
    SqlNode serializer =
