    ...
    public static Schema convertToSchema(LogicalType logicalType, String rowName) {
        ...
        switch (logicalType.getTypeRoot()) {
            ...
            case TIMESTAMP_WITH_TIME_ZONE:
                ...
                org.apache.avro.LogicalType zonedavroLogicalType;
                if (precision <= 3) {
                    zonedavroLogicalType = LogicalTypes.timestampMillis();
                } else {
                ...
                Schema zonedtimestamp = zonedavroLogicalType.addToSchema(SchemaBuilder.builder().longType());
                return nullable ? nullableSchema(zonedtimestamp) : zonedtimestamp;
            case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
                final LocalZonedTimestampType localtimestampType = (LocalZonedTimestampType) logicalType;
                precision = localtimestampType.getPrecision();
                org.apache.avro.LogicalType localavroLogicalType;
                if (precision <= 3) {
                    localavroLogicalType = LogicalTypes.timestampMillis();
                } else {
                    throw new IllegalArgumentException(
                        "Avro does not support TIMESTAMP type "
                            + "with precision: "
                ...
                Schema localtimestamp = localavroLogicalType.addToSchema(SchemaBuilder.builder().longType());
