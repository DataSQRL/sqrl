...
public class RecordShredderFlatMap implements FlatMapFunction<SourceRecord<Name>, Row> {
    ...
    public void flatMap(SourceRecord<Name> sourceRecord, Collector<Row> collector) throws Exception {
        ...
        constructRows(sourceRecord.getData(), cols, colno, 0, 0, collector);
    }
    ...
    private void constructRows(Map<Name, Object> data, Object[] cols, int colno, int depth, int arrayIndex,
        ...
        if (colno >= projections.length) {
            ...
            if (nested instanceof Map) {
                constructRows((Map) nested, cols, colno, depth + 1, 0, collector);
            } else if (nested instanceof List) {
                List arr = (List)nested;
                for (int i = 0; i < arr.size(); i++) {
                    Object[] colcopy = cols.clone();
                    constructRows((Map) arr.get(i), colcopy, colno, depth + 1, i, collector);
                }
